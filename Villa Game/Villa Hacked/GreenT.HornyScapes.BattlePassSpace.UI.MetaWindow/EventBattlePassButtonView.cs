...
public class EventBattlePassButtonView : MonoView
{
	...
	public void Set(CalendarModel eventCalendarModel, BattlePass battlePass)
	{
		...
		_calendarStateStream = ObservableExtensions.Subscribe<EntityStatus>((IObservable<EntityStatus>)eventCalendarModel.CalendarState, (Action<EntityStatus>)SetState);
		TrackPoints();
		Show();
	}
	...
	private void TrackPoints()
	{
		...
		if (_currencyProcessor.TryGetCountReactiveProperty(currency, out _currencyTarget))
		{
			...
			_currencyTargetStream = ObservableExtensions.Subscribe<int>((IObservable<int>)_currencyTarget, (Action<int>)UpdateLevel);
		}
	}
