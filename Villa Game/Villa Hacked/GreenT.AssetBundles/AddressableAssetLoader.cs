...
public class AddressableAssetLoader
{
	...
	{
		return Observable.FromCoroutine<Unit>((Func<IObserver<Unit>, CancellationToken, IEnumerator>)((IObserver<Unit> observer, CancellationToken cancellationToken) => PreloadAssetCoroutine(assetName, observer, cancellationToken)));
	}

	public IObservable<TAsset> LoadAsset<TAsset>(IResourceLocation location) where TAsset : UnityEngine.Object
	{
		return Observable.FromCoroutine<TAsset>((Func<IObserver<TAsset>, CancellationToken, IEnumerator>)((IObserver<TAsset> observer, CancellationToken cancellationToken) => LoadAssetCoroutine(location, observer, cancellationToken)));
	}

	public IObservable<TAsset> LoadAsset<TAsset>(string assetPath) where TAsset : UnityEngine.Object
	{
		return Observable.FromCoroutine<TAsset>((Func<IObserver<TAsset>, CancellationToken, IEnumerator>)((IObserver<TAsset> observer, CancellationToken cancellationToken) => LoadAssetCoroutine(assetPath, observer, cancellationToken)));
	}
	...
	{
		return Observable.ToList<TAsset>(Observable.SelectMany<IResourceLocation, TAsset>(Observable.ToObservable<IResourceLocation>(locations), (Func<IResourceLocation, IObservable<TAsset>>)LoadAsset<TAsset>));
	}
