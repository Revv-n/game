...
public class PostJsonRequest<TResponse, TValue> : IPostRequest<TResponse, TValue>, ICustomizablePostRequest<TResponse, TValue>
{
	...
	private IObservable<TResponse> RequestByUrl(string requestUrl, TValue fields, IDictionary<string, string> headers = null)
	{
		...
		return Observable.Catch<TResponse, Exception>(Observable.Select<string, TResponse>(HttpRequestExecutor.PostJSONRequest(requestUrl, jsonSend, headers), (Func<string, TResponse>)JsonConvert.DeserializeObject<TResponse>), (Func<Exception, IObservable<TResponse>>)delegate(Exception ex)
		{
