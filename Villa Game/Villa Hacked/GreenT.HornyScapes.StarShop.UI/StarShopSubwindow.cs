...
public class StarShopSubwindow : AnimatedSubwindow
{
	...
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Sprite>((IObservable<Sprite>)artManager.Current, (Action<Sprite>)delegate(Sprite _current)
		{
	...
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<StarShopArt>(Observable.SelectMany<StarShopArt, StarShopArt>(Observable.ToObservable<StarShopArt>(artManager.Collection.Where((StarShopArt _story) => !_story.Locker.IsOpen.Value)), (Func<StarShopArt, IObservable<StarShopArt>>)((StarShopArt _story) => Observable.Select<bool, StarShopArt>(Observable.Where<bool>((IObservable<bool>)_story.Locker.IsOpen, (Func<bool, bool>)((bool _isOpen) => _isOpen)), (Func<bool, StarShopArt>)((bool _) => _story)))), (Action<StarShopArt>)SetStory), (ICollection<IDisposable>)storyUnlockStream);
	}

	private void ManageComingSoonPlaceHolder()
	{
		comingSoonStream?.Dispose();
		comingSoonStream = ObservableExtensions.Subscribe<bool>(Observable.Select<IList<StarShopItem>, bool>(Observable.CombineLatest<StarShopItem>(starShopManager.Collection.Select((StarShopItem _item) => Observable.StartWith<StarShopItem>(_item.OnUpdate, _item))), (Func<IList<StarShopItem>, bool>)IsAnyTaskDisplayed), (Action<bool>)delegate(bool _anyTaskIsDisplayed)
		{
	...
	private void StarShopLoader()
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<StarShopItem>(Observable.TakeUntilDisable<StarShopItem>(Observable.Where<StarShopItem>(Observable.Merge<StarShopItem>(source.Select((StarShopItem _item) => _item.OnUpdate)), (Func<StarShopItem, bool>)((StarShopItem _item) => _item.State == EntityStatus.InProgress)), (Component)this), (Action<StarShopItem>)delegate(StarShopItem _item)
		{
	...
	{
		base.OnDestroy();
		CompositeDisposable obj = storyUnlockStream;
		if (obj != null)
		{
			obj.Dispose();
		}
