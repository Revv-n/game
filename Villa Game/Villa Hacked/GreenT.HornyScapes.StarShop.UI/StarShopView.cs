...
public class StarShopView : MonoView<IStarShopItem>
{
	...
	public IObservable<IStarShopItem> OnSet => Observable.AsObservable<IStarShopItem>((IObservable<IStarShopItem>)onSet);
	...
	{
		completeBtn?.Dispose();
		completeBtn = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(complete), (Action<Unit>)delegate
		{
			SetComplete();
		});
	}
	...
	{
		eventStream.Clear();
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<StarShopItem>(Observable.Where<StarShopItem>(item.OnUpdate, (Func<StarShopItem, bool>)((StarShopItem _item) => _item.State == EntityStatus.InProgress || _item.State == EntityStatus.Complete)), (Action<StarShopItem>)delegate(StarShopItem _item)
		{
			UpdateButtonState(_item);
		}), (ICollection<IDisposable>)eventStream);
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<StarShopItem>(Observable.Where<StarShopItem>(item.OnUpdate, (Func<StarShopItem, bool>)((StarShopItem _item) => _item.State == EntityStatus.Rewarded)), (Action<StarShopItem>)delegate
		{
