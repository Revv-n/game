...
public class ShopBundleLotFactory : LotFactory<ShopBundleMapper, BundleLot>
{
	...
	public override BundleLot Create(ShopBundleMapper mapper)
	{
		try
		{
			...
			LinkedContentAnalyticData linkedContentAnalyticData = analyticDataFactory.Create(CurrencyAmplitudeAnalytic.SourceType.Bought);
			LinkedContent reward = lootboxContentFactory.Create(mapper.lootbox_id, linkedContentAnalyticData);
