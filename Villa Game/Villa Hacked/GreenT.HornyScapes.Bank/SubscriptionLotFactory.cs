...
public class SubscriptionLotFactory : LotFactory<SubscriptionLotMapper, SubscriptionLot>
{
	...
	{
		if (lootboxIDs == null || LinqExtensions.IsEmpty<int>((IEnumerable<int>)lootboxIDs))
		{
			return null;
		}
		LinkedContentAnalyticData linkedContentAnalyticData = _analyticDataFactory.Create(CurrencyAmplitudeAnalytic.SourceType.None);
		LootboxLinkedContent lootboxLinkedContent = _lootboxContentFactory.Create(lootboxIDs.First(), linkedContentAnalyticData);
		if (lootboxIDs.Length <= 1)
		{
			return lootboxLinkedContent;
		}
		foreach (int num in lootboxIDs)
		{
			LinkedContentAnalyticData linkedContentAnalyticData2 = _analyticDataFactory.Create(CurrencyAmplitudeAnalytic.SourceType.None);
			LootboxLinkedContent content = _lootboxContentFactory.Create(num, linkedContentAnalyticData2);
