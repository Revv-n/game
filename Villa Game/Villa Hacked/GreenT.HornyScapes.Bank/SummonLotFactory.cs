...
public class SummonLotFactory : LotFactory<SummonMapper, SummonLot>
{
	...
	public override SummonLot Create(SummonMapper mapper)
	{
		...
		LinkedContentAnalyticData linkedContentAnalyticData = analyticDataFactory.Create(CurrencyAmplitudeAnalytic.SourceType.Summon);
		LinkedContent reward = lootboxContentFactory.Create(mapper.reward, linkedContentAnalyticData);
		LinkedContent firstPurchaseReward = (mapper.first_reward.HasValue ? lootboxContentFactory.Create(mapper.first_reward.Value, linkedContentAnalyticData) : null);
		...
		reward = (mapper.reward_10.HasValue ? lootboxContentFactory.Create(mapper.reward_10.Value, linkedContentAnalyticData) : null);
		firstPurchaseReward = (mapper.first_reward_10.HasValue ? lootboxContentFactory.Create(mapper.first_reward_10.Value, linkedContentAnalyticData) : null);
