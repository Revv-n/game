...
public abstract class BaseRestoreEnergyView<TEnergyRestore, TRestoreEnergyPopup> : EventEnergyBaseView where TEnergyRestore : BaseEnergyRestore where TRestoreEnergyPopup : BaseRestoreEnergyPopup
{
	...
	public void Set(TEnergyRestore mainEnergyRestore)
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<int>(Observable.SelectMany<Price<int>, int>(Observable.Do<Price<int>>(_mainEnergyRestore.PriceLogics.OnPriceUpdate, (Action<Price<int>>)delegate(Price<int> price)
		{
			statableButtonView.Set(price);
		}), (Func<Price<int>, IObservable<int>>)IsEnoughCurrencyStream), (Action<int>)statableButtonView.SetValueColor), (Component)this);
	...
	{
		return Observable.Select<int, int>((IObservable<int>)_currencyProcessor.GetCountReactiveProperty(price.Currency, price.CompositeIdentificator), (Func<int, int>)((int _) => GetButtonColorValue(price)));
	}
