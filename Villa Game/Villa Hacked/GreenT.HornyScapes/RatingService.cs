...
public sealed class RatingService
{
	...
	public IObservable<LeaderboardResponse> GetLeaderboard(RatingData ratingData, Action onError = null)
	{
		if (string.IsNullOrEmpty(ratingData.AuthorizationToken))
		{
			...
			return Observable.Return<LeaderboardResponse>((LeaderboardResponse)null);
		}
		return Observable.DoOnError<LeaderboardResponse>(_leaderboardRequest.GetRequest(ratingData.TargetRating.Range, ratingData.AuthorizationToken), (Action<Exception>)delegate
		{
			if (onError != null)
			{
				onError();
	...
	{
		return Observable.DoOnError<Response>(_leaderboardAddPointsRequest.Post(reason, token, amount), (Action<Exception>)delegate
		{
		});
	}
