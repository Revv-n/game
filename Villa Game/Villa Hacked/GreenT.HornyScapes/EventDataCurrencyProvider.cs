...
public class EventDataCurrencyProvider : IDisposable
{
	...
	{
		_eventChangeStream = ObservableExtensions.Subscribe<Event>(Observable.Select<(CalendarModel, Event), Event>((IObservable<(CalendarModel, Event)>)_eventProvider.CurrentCalendarProperty, (Func<(CalendarModel, Event), Event>)(((CalendarModel calendar, Event @event) tuple) => tuple.@event)), (Action<Event>)OnEventDataChanged);
	}
	...
	private void CreateEventDataDictionary()
	{
		...
		containers = new Dictionary<EventCurrencyType, EventCurrenciesActionContainer>
		{
			{
				EventCurrencyType.Core,
				new EventCurrenciesActionContainer(EventCurrencyType.Core, (IReadOnlyReactiveProperty<int>)(object)eventCurrencies[EventCurrencyType.Core], null)
			},
			{
				EventCurrencyType.XP,
				new EventCurrenciesActionContainer(EventCurrencyType.XP, (IReadOnlyReactiveProperty<int>)(object)eventCurrencies[EventCurrencyType.XP], null)
			}
		};
	}
	...
	{
		IDisposable value = ObservableExtensions.Subscribe<int>((IObservable<int>)@case.Wallet.Get(type), (Action<int>)delegate(int x)
		{
