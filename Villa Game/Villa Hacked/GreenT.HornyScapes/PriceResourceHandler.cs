...
public static class PriceResourceHandler
{
	...
	public static (CurrencyType, CompositeIdentificator) ParsePriceSourse(string origin)
	{
		...
		string[] array = origin.Split(':', StringSplitOptions.None);
		int[] array2 = null;
		return new ValueTuple<CurrencyType, CompositeIdentificator>(item2: new CompositeIdentificator((array.Length != 2) ? new int[1] : Array.ConvertAll(array[1].Split(',', StringSplitOptions.None), int.Parse)), item1: GetCurrencyType(array[0]));
	}
