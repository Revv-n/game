...
public class InputBlockController : IInputBlockerController, IDisposable
{
	...
	public IObservable<IInputBlocker> OnUpdate => Observable.AsObservable<IInputBlocker>((IObservable<IInputBlocker>)onUpdateBlocker);

	public void AddBlocker(IInputBlocker inputBlocker)
	{
		inputBlockers.Add(inputBlocker);
		IDisposable value = DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<IInputBlocker>(inputBlocker.OnUpdate, (Action<IInputBlocker>)onUpdateBlocker.OnNext), (ICollection<IDisposable>)trackStreams);
