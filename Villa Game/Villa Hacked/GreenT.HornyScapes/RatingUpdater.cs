...
public sealed class RatingUpdater : MonoBehaviour
{
	...
	private void StartOnEnableUpdateCooldown()
	{
		...
		_onEnableCooldownDisposable = ObservableExtensions.Subscribe<long>(Observable.Timer(TimeSpan.FromSeconds(_intConstants["leaderboard_onenable_cooldown"])), (Action<long>)delegate
		{
	...
	{
		_autoCooldownDisposable?.Dispose();
		_autoCooldownDisposable = ObservableExtensions.Subscribe<long>(Observable.Repeat<long>(Observable.Timer(TimeSpan.FromSeconds(_intConstants["leaderboard_autoupdate_cooldown"]))), (Action<long>)delegate
		{
			_ratingView.AutoUpdate();
		});
	}
