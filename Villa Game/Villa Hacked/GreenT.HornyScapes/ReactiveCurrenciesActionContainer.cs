...
public class ReactiveCurrenciesActionContainer : ICurrenciesActionContainer, IDisposable
{
	...
	public IReadOnlyReactiveProperty<int> ReactiveCount => (IReadOnlyReactiveProperty<int>)(object)mainBalance[currencyType, _currencyIdentificator].Count;

	public CurrencyAmplitudeAnalytic.SourceType LastSourceType { get; private set; }

	public IObservable<int> OnSpend()
	{
		return Observable.AsObservable<int>((IObservable<int>)spendStream);
	}

	public IObservable<int> OnAdd()
	{
		return Observable.AsObservable<int>((IObservable<int>)addStream);
	}
