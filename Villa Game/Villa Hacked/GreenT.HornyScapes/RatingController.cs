...
public sealed class RatingController : IController, IDisposable
{
	...
	{
		return ObservableExtensions.Subscribe<LeaderboardResponse>(_ratingService.GetLeaderboard(_targetRatingData, onError), (Action<LeaderboardResponse>)delegate(LeaderboardResponse response)
		{
			if (response != null)
			{
	...
	private void TryRegister()
	{
		...
		{
			ObservableExtensions.Subscribe<RegistrationResponse>(_ratingService.TryRegister(_targetRatingData), (Action<RegistrationResponse>)delegate(RegistrationResponse response)
			{
				if (response != null)
				{
	...
	private void TrackTime()
	{
		...
		_ratingTimeTrackDisposable = ObservableExtensions.Subscribe<GenericTimer>(Timer.OnTimeIsUp, (Action<GenericTimer>)delegate
		{
			_targetRatingData.IsFinished = true;
		});
	}
	...
	private void TrackCurrency()
	{
		...
		{
			_addStreamDisposable?.Dispose();
			_addStreamDisposable = ObservableExtensions.Subscribe<int>(_currenciesActionContainer.OnAdd(), (Action<int>)OnAddCurrency);
		}
	}
	...
	private void OnAddCurrency(int amount)
	{
		...
		{
			_addCurrencyRequestDisposable?.Dispose();
			_addCurrencyRequestDisposable = ObservableExtensions.Subscribe<Response>(_ratingService.AddScores(CurrencyAmplitudeAnalytic.Source[_currenciesActionContainer.LastSourceType], _targetRatingData.AuthorizationToken, amount), (Action<Response>)delegate
			{
				_localGetLeaderboardDisposable?.Dispose();
