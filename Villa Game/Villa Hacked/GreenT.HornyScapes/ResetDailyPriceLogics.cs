...
public class ResetDailyPriceLogics : IResetDailyPriceLogics
{
	...
	public IObservable<Price<int>> OnPriceUpdate => (IObservable<Price<int>>)_onPriceUpdate;
	...
	public void ResetTimer()
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<GenericTimer>(_timer.OnTimeIsUp, (Action<GenericTimer>)delegate
		{
			TryResetDailyInfo();
	...
	private void TryResetDailyInfo()
	{
		...
		if (!(date == DateTime.MinValue.Date) && !(date >= _clock.GetDate()))
		{
			ResetDailyInfo();
		}
	}
