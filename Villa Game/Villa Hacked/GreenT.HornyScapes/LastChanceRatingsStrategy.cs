...
public sealed class LastChanceRatingsStrategy : BaseLastChanceStrategy, IDisposable
{
	...
	public void Dispose()
	{
		CompositeDisposable initDisposables = _initDisposables;
		if (initDisposables != null)
		{
			initDisposables.Clear();
		}
		CompositeDisposable initDisposables2 = _initDisposables;
		if (initDisposables2 != null)
		{
			initDisposables2.Dispose();
		}
		CompositeDisposable collectDisposables = _collectDisposables;
		if (collectDisposables != null)
		{
			collectDisposables.Clear();
		}
		CompositeDisposable collectDisposables2 = _collectDisposables;
		if (collectDisposables2 != null)
		{
			collectDisposables2.Dispose();
		}
	}
	...
	public override void Init(LastChance lastChance, Action<LastChance> onFinish)
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<(EventBundleData, IAssetBundle)>(Observable.Do<(EventBundleData, IAssetBundle)>(Observable.Select<IAssetBundle, (EventBundleData, IAssetBundle)>(Observable.ContinueWith<EventBundleData, IAssetBundle>(Observable.Do<EventBundleData>(_eventBundleLoader.Load(eventMapper.event_bundle), (Action<EventBundleData>)delegate(EventBundleData _eventBundleData)
		{
			buffer._eventBundleData = _eventBundleData;
		}), Observable.Do<IAssetBundle>(_eventShopBundleLoader.Load(eventMapper.event_bundle), (Action<IAssetBundle>)delegate(IAssetBundle _shopBundle)
		{
			buffer._shopBundle = _shopBundle;
		})), (Func<IAssetBundle, (EventBundleData, IAssetBundle)>)((IAssetBundle _) => buffer)), (Action<(EventBundleData, IAssetBundle)>)delegate((EventBundleData _eventBundleData, IAssetBundle _shopBundle) _bundles)
		{
		...
		})), (ICollection<IDisposable>)_initDisposables);
	}
	...
	public override void Stop(LastChance lastChance)
	{
		...
		}), (ICollection<IDisposable>)_collectDisposables);
	}
