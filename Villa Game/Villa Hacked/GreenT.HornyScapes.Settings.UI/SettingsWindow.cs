...
public class SettingsWindow : PopupWindow
{
	...
	private void SubscribeRegisterBtn(int value)
	{
		if (PlatformHelper.IsEpochaMonetization() || PlatformHelper.IsHaremMonetization())
		{
			...
			{
			case 0:
				clickStream = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(button), (Action<Unit>)OpenRegistrationWindow);
				break;
			case 1:
				clickStream = ObservableExtensions.Subscribe<Unit>(Observable.TakeWhile<Unit>(UnityUIComponentExtensions.OnClickAsObservable(button), (Func<Unit, bool>)((Unit _) => !isRewardedForRegistration.Value)), (Action<Unit>)ClaimReward);
				break;
			}
		}
		if (!PlatformHelper.IsErolabsMonetization())
		...
		{
		case 0:
			clickStream = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(button), (Action<Unit>)delegate
			{
	...
	private void TrackRegistrationReward()
	{
		...
		openLootboxStream = ObservableExtensions.Subscribe<Lootbox>(Observable.Where<Lootbox>(Observable.TakeWhile<Lootbox>(lootboxOpener.OnOpen, (Func<Lootbox, bool>)((Lootbox _) => !isRewardedForRegistration.Value)), (Func<Lootbox, bool>)((Lootbox _lootbox) => lootboxRegistrationRewardID == _lootbox.ID)), (Action<Lootbox>)OnClaimRegistrationReward);
	}
	...
	private void OnEnable()
	{
		...
		userUpdateStream = ObservableExtensions.Subscribe<User>(userData.OnUpdate, (Action<User>)delegate
		{
			Setup();
		});
