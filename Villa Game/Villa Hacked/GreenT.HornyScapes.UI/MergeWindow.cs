...
public class MergeWindow : Window
{
	...
	{
		_disposables.Clear();
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<AnimationSetOpenCloseController>(Observable.DoOnCancel<AnimationSetOpenCloseController>(animations.Close(), (Action)base.Close), (Action<AnimationSetOpenCloseController>)delegate
		{
			InstantClose();
		}), (ICollection<IDisposable>)_disposables);
	}
