...
{
	private ReactiveProperty<bool> isVisible = new ReactiveProperty<bool>(false);
	...
	{
		IsVisible = (IReadOnlyReactiveProperty<bool>)(object)ReactivePropertyExtensions.ToReadOnlyReactiveProperty<bool>((IObservable<bool>)isVisible);
	}
	...
	private void OnAnyWindowChangeState(IWindow obj)
	{
		...
		{
			disposable?.Dispose();
			disposable = ObservableExtensions.Subscribe<long>(Observable.TimerFrame(1, (FrameCountType)2), (Action<long>)delegate
			{
				UpdateIndicator();
			});
		}
