...
public class CharacterProvider
{
	...
	{
		return Observable.Concat<ICharacter>(characterManager.Collection.Where((ICharacter _character) => charactersId.Contains(_character.ID)).Select(Get));
	}
	...
	public IObservable<ICharacter> Get(ICharacter emptyCharacter)
	{
		...
		return Observable.ToObservable<ICharacter>(emptyCharacter.AsEnumerable());
	}

	private IObservable<ICharacter> LoadCharacter(ICharacter _character)
	{
		return Observable.Select<CharacterData, ICharacter>(Observable.Do<CharacterData>(LoadBundleCharacter(_character).Debug("Character data loading: " + _character.ID), SetBundleToCharacter(_character)), (Func<CharacterData, ICharacter>)((CharacterData _) => _character));
	}
	...
	{
		return Observable.Concat<CharacterStories>(characterManager.Collection.Where((ICharacter _character) => charactersId.Contains(_character.ID)).Select(GetStory));
	}
	...
	{
		return Observable.Do<CharacterStories>(LoadStoryBundleCharacter(_character), SetStoryBundleToCharacter(_character));
	}
