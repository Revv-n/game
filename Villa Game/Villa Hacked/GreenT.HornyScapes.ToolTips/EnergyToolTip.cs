...
public class EnergyToolTip : AnimatedToolTipView<TailedToolTipSettings>
{
	...
	public override void Set(TailedToolTipSettings settings)
	{
		...
		_boosterSetupStream = ObservableExtensions.Subscribe<int>(ObserveExtensions.ObserveEveryValueChanged<IEnumerable<BoosterModel>, int>(_boosterStorage.Collection, (Func<IEnumerable<BoosterModel>, int>)((IEnumerable<BoosterModel> collection) => collection.Count()), (FrameCountType)0, false), (Action<int>)delegate
		{
			SetupBoosterInfo();
		});
		showAnimation.Init();
	...
	private void SetupButton()
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(_openSectionButton), (Action<Unit>)delegate
		{
			_contentSetter.Set();
