...
public class BattlePassPurchaseViewAdapter : MonoView
{
	...
	public void Init(LotManager lotManager, Purchaser purchaser, LocalizationService localizationService, WindowOpener windowOpener, ContentStorageProvider storageProvider, BattlePass battlePass, int[] lots)
	{
		...
		if (!_battlePass.Data.StartData.PremiumPurchasedProperty.Value)
		{
			...
			_purchaseRequestStream = ObservableExtensions.Subscribe<BundleLot>(SetupAdapters(lots), (Action<BundleLot>)Purchase);
		}
	}
	...
	private IObservable<BundleLot> SetupAdapters(int[] lots)
	{
		...
		foreach (int target in lots)
		{
			...
			ObservableExtensions.Subscribe<string>(Observable.Take<string>((IObservable<string>)_localizationService.ObservableText(_battlePass.Bundle.PremiumLevelBonus), 1), (Action<string>)delegate(string value)
			{
				adapter.SetLocalization(value);
			});
			observable = Observable.Merge<BundleLot>(observable, new IObservable<BundleLot>[1] { adapter.OnPurchase() });
			num++;
		}
		return observable;
	}
	...
	private void Purchase(BundleLot bundleLot)
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<bool>(_purchaser.OnResult, (Action<bool>)delegate(bool value)
		{
			OnPurchaseEnded(value, bundleLot);
		}), (ICollection<IDisposable>)_onPurchaseStream);
