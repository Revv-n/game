...
public class PremiumConditionsLock : MonoBehaviour, IConditionsLock, IDisposable
{
	...
	public void Initialization(IConditionReceivingReward conditionReceiving)
	{
		...
		{
			subscribe = ObservableExtensions.Subscribe<ConditionState>(Observable.Where<ConditionState>((IObservable<ConditionState>)conditionReceiving.State, (Func<ConditionState, bool>)IsDisabled), (Action<ConditionState>)delegate
			{
				OnCompleted();
			});
		}
