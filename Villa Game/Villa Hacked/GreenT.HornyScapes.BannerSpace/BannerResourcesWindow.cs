...
public class BannerResourcesWindow : Window
{
	...
	{
		button.Set(value);
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<int>(button.OnClick, (Action<int>)ChangeCount), (ICollection<IDisposable>)_disposables);
	}

	private void SubscribeInput()
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<string>(UnityEventExtensions.AsObservable<string>((UnityEvent<string>)_resourcesText.onValueChanged), (Action<string>)OnValueChanged), (ICollection<IDisposable>)_disposables);
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(UnityEventExtensions.AsObservable((UnityEvent)_priceButton.onClick), (Action<Unit>)delegate
		{
			TryBuy();
