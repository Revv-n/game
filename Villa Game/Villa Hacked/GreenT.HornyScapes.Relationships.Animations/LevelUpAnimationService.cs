...
public class LevelUpAnimationService : MonoView
{
	...
	public void PlayAnimation(int relationshipId, int index)
	{
		...
		_closeStream = ObservableExtensions.Subscribe<long>(Observable.ObserveOnMainThread<long>(Observable.Timer(TimeSpan.FromSeconds(_closeDelay))), (Action<long>)delegate
		{
			SetCloseStream();
		});
	}
	...
	{
		_closeStream?.Dispose();
		_closeStream = ObservableExtensions.Subscribe<long>(Observable.Where<long>(Observable.EveryUpdate(), (Func<long, bool>)((long _) => Input.GetKeyDown(KeyCode.Mouse0))), (Action<long>)delegate
		{
			CloseWindow();
		});
	}
