...
public class MergeCreateTweenBuilder : MonoBehaviour
{
	...
	public Tween BuildTweeen(SpriteRenderer drag, SpriteRenderer placed, GameItem created)
	{
		...
		sequence.Append(drag.transform.DOMove(placed.transform.position, downScaleTime).SetEase(downEase)).Join(drag.transform.DOScale(downScaleSize, downScaleTime).SetEase(downEase)).Join(DOTweenModuleSprite.DOFade(drag, downFade, downScaleTime).SetEase(downEase))
			.Join(placed.transform.DOScale(downScaleSize, downScaleTime).SetEase(downEase))
			.Join(DOTweenModuleSprite.DOFade(placed, downFade, downScaleTime).SetEase(downEase));
