...
public class PhotoView : MediaView<ImageMedia>
{
	...
	private void SetupFavoriteToggle(ImageMedia media)
	{
		...
		favoriteToggleStream = (favoriteToggleStream = DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<bool>(UnityUIComponentExtensions.OnValueChangedAsObservable(favoriteToggle), (Action<bool>)delegate(bool _value)
		{
	...
	private void SetupButton(ImageMedia media)
	{
		...
		clickStream = DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(Button), (Action<Unit>)delegate
		{
			OpenPhoto(media);
		}), (Component)this);
	}
