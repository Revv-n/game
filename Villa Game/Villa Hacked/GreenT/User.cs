...
public class User : IEquatable<User>, IDisposable
{
	...
	public IObservable<User> OnUpdate => (IObservable<User>)onUpdate;

	public ISubject<User> OnLogin => (ISubject<User>)(object)onLogin;
	...
	{
		return Observable.Where<User>((IObservable<User>)OnLogin, (Func<User, bool>)((User user) => !user.Type.Equals(State.Unknown)));
	}
