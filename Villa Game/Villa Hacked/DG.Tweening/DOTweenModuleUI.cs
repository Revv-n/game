...
public static class DOTweenModuleUI
{
	...
	public static Sequence DOGradientColor(this Image target, Gradient gradient, float duration)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			sequence.Append(DOColor(target, gradientColorKey.color, duration2).SetEase(Ease.Linear));
		}
	...
	{
		return DOTween.Shake((DOGetter<Vector3>)(() => target.anchoredPosition), (DOSetter<Vector3>)delegate(Vector3 x)
		{
			target.anchoredPosition = x;
	...
	public static Tweener DOShakeAnchorPos(this RectTransform target, float duration, Vector2 strength, int vibrato = 10, float randomness = 90f, bool snapping = false, bool fadeOut = true)
	{
		...
		{
			target.anchoredPosition = x;
		}, duration, (Vector3)strength, vibrato, randomness, fadeOut).SetTarget(target).SetSpecialStartupMode(SpecialStartupMode.SetShake)
			.SetOptions(snapping);
	}
