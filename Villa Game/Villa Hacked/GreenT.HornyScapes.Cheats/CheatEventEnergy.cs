...
public class CheatEventEnergy : MonoBehaviour, IDisposable
{
	...
	private void InnerInit(ICurrencyProcessor currencyProcessor)
	{
		...
		Dispose();
		_addCurrencyDisposable = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(AddEventEnergyCurrency), (Action<Unit>)delegate
		{
			OnAddEventEventCurrency();
		});
		_spendCurrencyDisposable = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(SpendEventEventCurrency), (Action<Unit>)delegate
		{
