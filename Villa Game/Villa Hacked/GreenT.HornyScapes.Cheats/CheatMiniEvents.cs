...
public class CheatMiniEvents : MonoBehaviour, IDisposable
{
	...
	private void InnerInit(ICurrencyProcessor currencyProcessor)
	{
		...
		Dispose();
		_addCurrencyDisposable = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(AddMiniEventCurrency), (Action<Unit>)delegate
		{
			OnAddMiniEventCurrency();
		});
		_spendCurrencyDisposable = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(SpendMiniEventCurrency), (Action<Unit>)delegate
		{
