...
public class BoundedReactiveProperty<T> : ReactiveProperty<T>, ISerializable where T : struct, IComparable, IComparable<T>, IEquatable<T>
{
	...
	protected ISubject<T> maxBoundChanged = (ISubject<T>)(object)new Subject<T>();

	protected ISubject<T> minBoundChanged = (ISubject<T>)(object)new Subject<T>();

	public IObservable<T> MaxBoundChanged => Observable.AsObservable<T>((IObservable<T>)maxBoundChanged);

	public IObservable<T> MinBoundChanged => Observable.AsObservable<T>((IObservable<T>)minBoundChanged);
	...
	public bool SetBounds(T upperBound, T lowerBound = default(T))
	{
		...
		if (!Max.Equals(max))
		{
			((IObserver<T>)maxBoundChanged).OnNext(Max);
		}
		if (!Min.Equals(min))
		{
			((IObserver<T>)minBoundChanged).OnNext(Min);
		}
		return true;
	}
	...
	protected BoundedReactiveProperty(SerializationInfo info, StreamingContext context)
	{
		...
		T value = (T)info.GetValue("Value", typeof(T));
		base.SetValue(value);
