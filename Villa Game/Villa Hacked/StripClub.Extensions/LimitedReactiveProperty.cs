...
public class LimitedReactiveProperty : ReactiveProperty<int>
{
	...
	public void SetTimer(Func<DateTime> getTime)
	{
		...
		watcher?.Dispose();
		watcher = ObservableExtensions.Subscribe<long>(Observable.TakeWhile<long>(Observable.Interval(TimeSpan.FromSeconds(1.0)), (Func<long, bool>)((long _) => getTime != null)), (Action<long>)delegate
		{
			ResetLimits();
		});
	}
