...
public static class SelectorTools
{
	public static Selector CreateSelector(string selectorString)
	{
		...
		string[] array = selectorString.Split(':', StringSplitOptions.None);
		CompositeIdentificator identificator;
		if (array.Length == 2)
		{
			int[] identificators = Array.ConvertAll(array[1].Split(',', StringSplitOptions.None), int.Parse);
		...
		string[] array2 = selectorString.Split('&', StringSplitOptions.None);
		if (array2.Length == 2)
		{
			string text = array2[0].Split(':', StringSplitOptions.None)[1].FirstCharToUpper();
			string obj = array2[1].Split(':', StringSplitOptions.None)[1];
