...
public class PhraseContainer : MonoBehaviour
{
	...
	public void ApplyPhrase(Phrase phrase, float waitTime)
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<string>((IObservable<string>)localizationService.ObservableText(phrase.Name), (Action<string>)delegate(string text)
		{
			_name.text = text;
		}), (ICollection<IDisposable>)_localizationDisposables);
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<string>((IObservable<string>)localizationService.ObservableText(phrase.Text), (Action<string>)delegate(string text)
		{
			_currentPhraseText = text;
			_text.text = text;
		}), (ICollection<IDisposable>)_localizationDisposables);
		textTyping = ObservableExtensions.Subscribe<Unit>(Observable.FromCoroutine((Func<IEnumerator>)(() => TypeText(_text, _currentPhraseText, waitTime)), false), (Action<Unit>)delegate
		{
