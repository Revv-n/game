...
public class CharacterSkinLoader : IBundlesLoader<int, SkeletonAnimation>, ILoader<int, SkeletonAnimation>
{
	...
	IObservable<SkeletonAnimation> ILoader<int, SkeletonAnimation>.Load(int bundleName)
	{
		...
		return Observable.Select<GameObject, SkeletonAnimation>(AssetsLoaderUtilities<GameObject>.LoadAssetByName(_addressablesService, () => GetPath(bundleName), name), (Func<GameObject, SkeletonAnimation>)((GameObject x) => x.GetComponent<SkeletonAnimation>()));
	}
