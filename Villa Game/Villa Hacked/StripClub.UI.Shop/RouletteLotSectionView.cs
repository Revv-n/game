...
public sealed class RouletteLotSectionView : BaseAbstractLotSectionView<BankTab, RouletteBankSummonLot>
{
	...
	protected override void TrackLotLockers(IEnumerable<RouletteBankSummonLot> allSectionLots)
	{
		...
		if (list.Any())
		{
			_updateStream = DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<bool>(Observable.Skip<bool>(Observable.Merge<bool>((IEnumerable<IObservable<bool>>)list), list.Count), (Action<bool>)delegate
			{
				Set(base.Source);
			}), (Component)this);
		}
