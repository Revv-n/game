...
public class SummonLotView : LotView
{
	...
	public override void Set(Lot lot)
	{
		...
		{
			DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<string>((IObservable<string>)_localizationService.ObservableText(summonLot.TitleKey), (Action<string>)delegate(string text)
			{
				textField.text = text;
		...
		foreach (TextMeshProUGUI textField in array)
		{
			DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<string>((IObservable<string>)_localizationService.ObservableText(summonLot.DescriptionKey), (Action<string>)delegate(string text)
			{
				textField.text = text;
			}), (ICollection<IDisposable>)_localizationDisposables);
		}
	...
	private void SetTimer(SummonLot summonLot)
	{
		if (!summonLot.GetUnlockTime().HasValue)
		{
			...
			DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(summonLot.OnUnlockTimeUpdate(), (Action<Unit>)delegate
			{
				SetTimer(summonLot);
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<GenericTimer>(timer.Timer.OnTimeIsUp, (Action<GenericTimer>)OnTimeIsUp), (ICollection<IDisposable>)disposables);
	}
	...
	private void SetupButton(PriceWithFreeView view, Price<int> price, Button purchaseButton, bool wholesale, bool isAttentionIconsOn)
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<int>(Observable.Select<int, int>((IObservable<int>)currencyProcessor.GetCountReactiveProperty(price.Currency), (Func<int, int>)((int _) => CurrencyProcessor.IsEnough(cost) ? 1 : 0)), (Action<int>)view.SetValueColor), (ICollection<IDisposable>)disposables);
		IConnectableObservable<bool> obj = Observable.Publish<bool>(Observable.Select<int, bool>(Observable.WithLatestFrom<Unit, int, int>(UnityUIComponentExtensions.OnClickAsObservable(purchaseButton), (IObservable<int>)currencyProcessor.GetCountReactiveProperty(price.Currency), (Func<Unit, int, int>)((Unit _, int _count) => _count)), (Func<int, bool>)((int _) => CurrencyProcessor.IsEnough(cost))));
