...
public class EventButtonSubscription : MonoView<CalendarModel>
{
	...
	public override void Set(CalendarModel source)
	{
		...
		if (_eventSettingsProvider.TryGetEvent(base.Source.BalanceId, out var @event))
		{
			...
			_event = @event;
			_eventClickStream = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(eventStartButton), (Action<Unit>)delegate
			{
	...
	private void Awake()
	{
		_openWindowStream = ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(button), (Action<Unit>)delegate
		{
			OpenWindow();
		});
	}
