...
public sealed class LastChanceEventBattlePassStrategy : BaseLastChanceStrategy, IDisposable
{
	...
	public override void Init(LastChance lastChance, Action<LastChance> onFinish)
	{
		...
		if (!_eventBundlesProvider.Contains(lastChance.EventId))
		{
			...
			if (@event == null)
			{
				DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<BattlePass>(Observable.SelectMany<EventBundleData, BattlePass>(_bundleLoader.Load<EventBundleData>(BundleType.EventShop, eventMapper.event_bundle), (Func<EventBundleData, IObservable<BattlePass>>)((EventBundleData bundle) => Observable.Do<BattlePass>(_battlePassChecker.LoadBattlePass(battlePassId), (Action<BattlePass>)delegate
				{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<CalendarModel>(_calendarQueue.OnCalendarActive(), (Action<CalendarModel>)delegate(CalendarModel model)
		{
	...
	public override void Stop(LastChance lastChance)
	{
		...
		{
			DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<BattlePass>(_battlePassChecker.LoadBattlePass(battlePassId), (Action<BattlePass>)delegate(BattlePass battlePass)
			{
				CheckBattlePass(battlePassId, battlePass, lastChance);
			}), (ICollection<IDisposable>)_stopDisposables);
		}
		else
		{
	...
	public void Dispose()
	{
		...
		CompositeDisposable stopDisposables = _stopDisposables;
		if (stopDisposables != null)
		{
			stopDisposables.Clear();
		}
		CompositeDisposable stopDisposables2 = _stopDisposables;
		if (stopDisposables2 != null)
		{
			stopDisposables2.Dispose();
		}
	}
