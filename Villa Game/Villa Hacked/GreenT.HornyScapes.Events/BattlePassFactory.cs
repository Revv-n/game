...
public class BattlePassFactory : IFactory<BattlePassMapper, BattlePassBundleData, BattlePassCategory, BattlePass>, IFactory
{
	...
	public BattlePass Create(BattlePassMapper mapper, BattlePassBundleData bundle, BattlePassCategory battlePassCategory)
	{
		...
		List<(int, RewardWithManyConditions)> first = CreateMergedRewards((IReadOnlyReactiveProperty<bool>)(object)unlockProperty, mapper, bundle, battlePassCategory, progressPrice, currencyType).ToList();
		List<(int, RewardWithManyConditions)> freeRewards = (from tuple in first.Concat(CreateRewardsFree((IReadOnlyReactiveProperty<bool>)(object)unlockProperty, mapper, bundle, battlePassCategory, progressPrice, currencyType))
			orderby tuple.Item1
			select tuple).ToList();
		List<(int, RewardWithManyConditions)> premiumRewards = (from tuple in first.Concat(CreateRewardsPremium((IReadOnlyReactiveProperty<bool>)(object)unlockProperty, mapper, bundle, battlePassCategory, progressPrice, currencyType))
			orderby tuple.Item1
			select tuple).ToList();
	...
	private List<Sprite> GetBattlePassView(string[] presets)
	{
		...
		{
			string[] array = presets[i].Split(':', StringSplitOptions.None);
			ContentSource contentSource = ((array.Length < 2) ? ContentSource.BattlePass : Enum.Parse<ContentSource>(array[0]));
			Sprite item = _assetProvider.FindBundleImageOrFake(contentSource, array[array.Length - 1]);
			list.Add(item);
		}
		return list;
	}
	...
	{
		string text = string.Join(",", mapper.any_lot_bought);
		return _lockerFactory.Create(UnlockType.AnyLotBought, text, LockerSourceType.BattlePass);
	}
	...
	private IEnumerable<(int, RewardWithManyConditions)> CreateRewards(IReadOnlyReactiveProperty<bool> unlockProperty, BattlePassBundleData bundle, BattlePassCategory battlePassCategory, CurrencyType currencyType, int[] targetLevels, string[] rewId, RewType[] rewType, int[] rewQTY, int[] blockLevels, long[] blockDate, int id, Dictionary<int, int> progressPrice, bool isPremium, int bpId, int[] isLights, params int[] mergedLevels)
	{
		...
		try
		{
			for (int j = 0; j < array.Length; j++)
			{
				num2 = j;
				num = array[j];
				int i2 = (mergedLevels.Any() ? MiscExtensions.IndexOf<int>((IList<int>)targetLevels, mergedLevels[j]) : j);
	...
	private List<IGrouping<T, T>> FillGroupedProperty<T>(IEnumerable<IGrouping<int, int>> groupedLevel, int[] targetLevels, T[] rewData)
	{
		...
		{
			int count = MiscExtensions.IndexOf<int>((IList<int>)targetLevels, item2.Key);
	...
	{
		if (LinqExtensions.IsEmpty<int>((IEnumerable<int>)mapper.merged_levels))
		{
