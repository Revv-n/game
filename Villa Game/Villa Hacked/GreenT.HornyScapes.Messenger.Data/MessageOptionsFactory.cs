...
public class MessageOptionsFactory
{
	...
	private void CreatCurrencyItemLot(PlayerMessageConfigMapper.ItemMapper itemMapper, List<IItemLot> itemNeccessaryForResponse, CurrencySelector selector)
	{
		...
		if (_gameSettings.CurrencySettings.TryGetValue(currency, out var currencySettings, identificator))
		{
			...
			IObservable<Sprite> updateAlternativeSprite = ObserveExtensions.ObserveEveryValueChanged<CurrencySettings, Sprite>(_gameSettings.CurrencySettings[currency, identificator], (Func<CurrencySettings, Sprite>)((CurrencySettings actualSettings) => actualSettings.AlternativeSprite), (FrameCountType)0, false);
