...
public sealed class Dialogue : IDisposable
{
	...
	public IObservable<Dialogue> OnUpdate => (IObservable<Dialogue>)onUpdate;
	...
	private void SetCompleteOnLastMessagePassed()
	{
		...
		{
			DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<ChatMessage>(Observable.FirstOrDefault<ChatMessage>(LastMessage.OnUpdate, (Func<ChatMessage, bool>)((ChatMessage _message) => _message.State.Contains(ChatMessage.MessageState.Passed))), (Action<ChatMessage>)delegate
			{
				IsComplete = true;
			}), (ICollection<IDisposable>)lastMessageTrackStream);
		}
		else
		{
			IsComplete = true;
	...
	{
		onUpdate?.Dispose();
		CompositeDisposable obj = lastMessageTrackStream;
		if (obj != null)
		{
			obj.Dispose();
		}
