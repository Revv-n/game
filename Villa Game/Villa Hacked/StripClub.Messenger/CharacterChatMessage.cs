...
public sealed class CharacterChatMessage : ChatMessage
{
	...
	protected override void OnAddFlag(MessageState state)
	{
		...
		{
			onAddFlag = ObservableExtensions.Subscribe<long>(Observable.Take<long>(Observable.Timer(TimeSpan.FromSeconds(1.0)), 1), (Action<long>)delegate
			{
				base.State |= MessageState.Passed;
				onUpdate.OnNext((ChatMessage)this);
			});
		}
	}
