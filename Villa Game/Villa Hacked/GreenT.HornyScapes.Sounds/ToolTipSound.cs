...
public abstract class ToolTipSound : MonoBehaviour
{
	...
	protected void InnerInit(IAudioPlayer audioPlayer)
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(OnOpenTrigger(), (Action<Unit>)delegate
		{
			OnOpen();
		}), (ICollection<IDisposable>)openCloseStream);
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(OnCloseTrigger(), (Action<Unit>)delegate
		{
			OnClose();
		}), (ICollection<IDisposable>)openCloseStream);
	}
