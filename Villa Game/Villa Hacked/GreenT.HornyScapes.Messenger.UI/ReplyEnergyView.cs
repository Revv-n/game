...
public class ReplyEnergyView : MonoBehaviour
{
	...
	private void DisplayTimeUntilNextRestore()
	{
		...
		counterStream = DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<long>(Observable.DoOnCompleted<long>(Observable.TakeWhile<long>(Observable.Timer(TimeSpan.Zero, messages.CheckFrequency, Scheduler.MainThreadIgnoreTimeScale), (Func<long, bool>)((long _) => messages.TimeUntilNextRestore > TimeSpan.Zero)), (Action)delegate
		{
