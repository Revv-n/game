...
public class ReplyExtenderWindow : PopupWindow
{
	...
	public IObservable<int> OnRefill => Observable.AsObservable<int>((IObservable<int>)onRefill);

	public IObservable<int> OnUpdateReplyExtensionCount => Observable.AsObservable<int>((IObservable<int>)onUpdateReplyExtensionCount);
	...
	private void SetupExtendButton()
	{
		...
		IObservable<bool> observable = Observable.Share<bool>(Observable.Select<Unit, bool>(UnityUIComponentExtensions.OnClickAsObservable(extendBtn), (Func<Unit, bool>)((Unit _) => IsReplyExtensionAvailable(out extendPrice, out playersCurrency))));
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<bool>(Observable.Where<bool>(observable, (Func<bool, bool>)((bool _isAvailable) => _isAvailable)), (Action<bool>)delegate
		{
			ExtendReplies();
	...
	private void SetupRefillButton(BoundedValue<int> Energy)
	{
		...
		IObservable<bool> observable = Observable.Share<bool>(Observable.Select<Unit, bool>(UnityUIComponentExtensions.OnClickAsObservable(refillBtn), (Func<Unit, bool>)((Unit _) => IsRefillAvailable(out refillPrice, out playersCurrency))));
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<bool>(Observable.Where<bool>(observable, (Func<bool, bool>)((bool _isAvailable) => _isAvailable)), (Action<bool>)delegate
		{
			Refill();
		}), (ICollection<IDisposable>)disposables);
	...
	private void ExtendReplies()
	{
		...
		{
			ReactiveProperty<int> obj = playersCurrency;
			obj.Value -= replyExtensionPrice.Value;
