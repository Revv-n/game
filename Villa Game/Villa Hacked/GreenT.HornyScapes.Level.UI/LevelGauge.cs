...
public class LevelGauge : MonoBehaviour
{
	...
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<int>(Observable.TakeUntilDisable<int>(playerExpController.OnProgressUpdate, (Component)this), (Action<int>)delegate(int value)
		{
			SetProgress(value, playerExpController.Target.Value);
		}), (Component)this);
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<float>(Observable.TakeUntilDisable<float>(levelProgressBar.OnProgressComplete, (Component)this), (Action<float>)delegate
		{
	...
	private void SetProgress(int current, int target)
	{
		...
		progressSequence = DOTween.Sequence().Append(base.transform.DOScale(startScale * fillScalePower, fillAnimationTime)).Join(DOTweenModuleUI.DOFade(glow, fillGlowAlpha, fillAnimationTime));
	}

	private void PlayDefaultAnimation()
	{
		animationSequence?.Kill();
		animationSequence = DOTween.Sequence().Append(base.transform.DOScale(startScale, UnFillAnimationTime)).Join(DOTweenModuleUI.DOColor(glow, startGlow, UnFillAnimationTime))
	...
	private void PlayReadyToSellAnimation()
	{
		...
		animationSequence?.Kill();
		progressSequence?.Kill();
		animationSequence = DOTween.Sequence().Join(glow.transform.DOScale(glowReadyToSellScale, readyToSellScaleTime)).Join(DOTweenModuleUI.DOFade(glow, 1f, readyToSellScaleTime))
