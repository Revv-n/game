...
public class LoginWindow : PopupWindow
{
	...
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(_loginButton.Button), (Action<Unit>)LoginRequest), (ICollection<IDisposable>)_disposables);
		_registrationWindow = windowsManager.Get<RegistrationWindow>();
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<Unit>(UnityUIComponentExtensions.OnClickAsObservable(_registerButton), (Action<Unit>)delegate
		{
