...
public class PromoteAnyGirlToLvlTrackService : IntTrackService, IDisposable
{
	...
	public override void Track()
	{
		...
		anyPromoteStream = ObservableExtensions.Subscribe<ICard>(Observable.TakeWhile<ICard>(_cardsCollectionTracker.GetAnyPromoteStream().Debug("GetAnyPromoteStream"), (Func<ICard, bool>)((ICard _) => !IsComplete())), (Action<ICard>)delegate
		{
			try
			{
				UpdateStats();
