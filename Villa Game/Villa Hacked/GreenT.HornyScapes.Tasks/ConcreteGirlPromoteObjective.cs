...
public sealed class ConcreteGirlPromoteObjective : AnyGirlPromoteObjective
{
	...
	public override void Track()
	{
		...
		anyPromoteStream = ObservableExtensions.Subscribe<(ICard, int)>(Observable.Where<(ICard, int)>(cardsCollectionTracker.GetConcretePromoteStream(), (Func<(ICard, int), bool>)(((ICard, int) value) => value.Item1.ContentType == contentType && value.Item1.ID == GirlID && value.Item2 >= TargetLevel)), (Action<(ICard, int)>)delegate
		{
			AddProgress();
		});
	}
