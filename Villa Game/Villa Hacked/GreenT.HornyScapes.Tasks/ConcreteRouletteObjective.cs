...
public sealed class ConcreteRouletteObjective : GainObjective, IDisposable
{
	...
	public override void Track()
	{
		base.Track();
		_signalStream?.Dispose();
		_signalStream = ObservableExtensions.Subscribe<RouletteLot>(Observable.Select<RouletteLotBoughtSignal, RouletteLot>(Observable.Where<RouletteLotBoughtSignal>(_signalBus.GetStream<RouletteLotBoughtSignal>(), (Func<RouletteLotBoughtSignal, bool>)delegate(RouletteLotBoughtSignal _lot)
		{
			RouletteLot lot = _lot.Lot;
			return lot != null && lot.ID == RouletteId;
		}), (Func<RouletteLotBoughtSignal, RouletteLot>)((RouletteLotBoughtSignal _lot) => _lot.Lot)), (Action<RouletteLot>)delegate(RouletteLot _lot)
		{
