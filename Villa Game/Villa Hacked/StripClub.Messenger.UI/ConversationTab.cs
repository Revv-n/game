...
public sealed class ConversationTab : ConversationView
{
	...
	public override void Set(Conversation conversation)
	{
		...
		updateStream = DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<MessengerUpdateArgs>(Observable.Where<MessengerUpdateArgs>(messenger.OnUpdate, (Func<MessengerUpdateArgs, bool>)delegate(MessengerUpdateArgs _args)
		{
		...
		}), (Action<MessengerUpdateArgs>)delegate
		{
			UpdateView(conversation);
		}), (Component)this);
	...
	private void UpdateView(Conversation conversation)
	{
		...
		if (characterChatMessage != null)
		{
			...
			_overviewDisposable = ObservableExtensions.Subscribe<string>((IObservable<string>)_localizationService.ObservableText(_overviewKey), (Action<string>)delegate(string text)
			{
				SetOverview(text);
			});
		}
