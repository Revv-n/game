...
public class NewMessageIndicator : MonoBehaviour
{
	...
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<MessengerUpdateArgs>(Observable.Where<MessengerUpdateArgs>(Observable.Where<MessengerUpdateArgs>(Observable.TakeUntilDisable<MessengerUpdateArgs>(messenger.OnUpdate, (Component)this), (Func<MessengerUpdateArgs, bool>)((MessengerUpdateArgs _args) => _args.Message != null)), (Func<MessengerUpdateArgs, bool>)((MessengerUpdateArgs _args) => _args.Message.State.Contains(ChatMessage.MessageState.Delivered) && !_args.Message.State.Contains(ChatMessage.MessageState.Read))), (Action<MessengerUpdateArgs>)delegate
		{
	...
	private void AppearanceAnimation()
	{
		...
		{
			DOTween.Sequence().OnStart(AnimationInitialization).Append(DOTweenModuleUI.DOSizeDelta(rectTransform, messageAwakeSize, appearanceDuraion))
				.Append(DOTweenModuleUI.DOFade(flare, 1f, flareAppearanceDuration))
				.Join(flare.transform.DOScale(flareAwakeScale, flareAppearanceDuration))
				.Append(ShortcutExtensions.DOShakeScale(flare.transform, flareAnimationDuration, flareShakeSettings.x, (int)flareShakeSettings.y, flareShakeSettings.z, true))
				...
				.Join(DOTweenModuleUI.DOFade(flare, 0f, flareAppearanceDuration))
				...
				.Append(DOTweenModuleUI.DOSizeDelta(rectTransform, new Vector2(0f, messageAwakeSize.y), appearanceDuraion))
				.OnComplete(delegate
				{
					base.gameObject.SetActive(value: false);
				});
