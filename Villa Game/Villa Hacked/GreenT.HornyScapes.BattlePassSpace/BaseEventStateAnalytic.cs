...
public abstract class BaseEventStateAnalytic<T> : IInitializable, IDisposable
{
	...
	{
		_eventStartDownload = ObservableExtensions.Subscribe<int>(_eventStateService.OnStartDownloadTuple(_calendarLoader), (Action<int>)OnEventStartDownload);
		_startTracker = ObservableExtensions.Subscribe<(T, CalendarModel)>(_eventStateService.OnEndDownloadTuple(_calendarLoader), (Action<(T, CalendarModel)>)OnEventDownloaded);
		_endTracker = ObservableExtensions.Subscribe<(T, CalendarModel)>(_eventStateService.OnEndTuple(), (Action<(T, CalendarModel)>)OnEventEnd);
	}
