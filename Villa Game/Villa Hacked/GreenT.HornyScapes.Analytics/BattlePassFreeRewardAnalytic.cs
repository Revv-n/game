...
public class BattlePassFreeRewardAnalytic : BaseEntityAnalytic<(CalendarModel calendar, BattlePass battlePass, RewardWithManyConditions reward)>
{
	...
	public BattlePassFreeRewardAnalytic(IAmplitudeSender<AmplitudeEvent> amplitude, CalendarQueue calendarQueue, BattlePassSettingsProvider battlePassSettingsProvider)
		: base(amplitude)
	{
	...
	public override void SendEventByPass((CalendarModel calendar, BattlePass battlePass, RewardWithManyConditions reward) tuple)
	{
		if (_rewardCounts.Count != 0)
		{
			...
			BattlePassFreeRewardAmplitudeEvent battlePassFreeRewardAmplitudeEvent = new BattlePassFreeRewardAmplitudeEvent(tuple2.Item1, tuple2.Item2, tuple2.Item3);
			((IAnalyticSender<AmplitudeEvent>)(object)amplitude).AddEvent((AmplitudeEvent)battlePassFreeRewardAmplitudeEvent);
		}
	}
	...
	private void FillList((CalendarModel calendar, BattlePass battlePass, RewardWithManyConditions reward) tuple)
	{
		...
		int item = tuple.battlePass.FreeRewardContainer.Rewards.Count((RewardWithManyConditions x) => x.IsRewarded || x.IsComplete);
		_rewardCounts.Enqueue((uniqID, iD, item));
	}

	private IObservable<(CalendarModel calendar, BattlePass battlePass, RewardWithManyConditions reward)> OnFreeRewardUpdate(CalendarModel calendarModel)
	{
		BattlePass battlePass = _battlePassSettingsProvider.GetBattlePass(calendarModel.BalanceId);
		return Observable.Select<RewardWithManyConditions, (CalendarModel, BattlePass, RewardWithManyConditions)>(Observable.Where<RewardWithManyConditions>(Observable.TakeWhile<RewardWithManyConditions>(battlePass.FreeRewardContainer.OnRewardUpdate(), (Func<RewardWithManyConditions, bool>)((RewardWithManyConditions _) => _calendarQueue.IsCalendarActive(calendarModel))), (Func<RewardWithManyConditions, bool>)((RewardWithManyConditions reward) => reward.State.Value == EntityStatus.Complete)), (Func<RewardWithManyConditions, (CalendarModel, BattlePass, RewardWithManyConditions)>)((RewardWithManyConditions reward) => (calendarModel: calendarModel, battlePass: battlePass, reward: reward)));
	}
}
