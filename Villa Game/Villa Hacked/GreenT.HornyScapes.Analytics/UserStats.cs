...
public sealed class UserStats : ISavableState, IInitializable, IDisposable
{
	...
	public IObservable<UserStats> OnUpdate => Observable.AsObservable<UserStats>((IObservable<UserStats>)onUpdate);
	...
	{
		disposable?.Dispose();
		IObservable<Unit> observable = Observable.Share<Unit>(Observable.AsUnitObservable<bool>(Observable.FirstOrDefault<bool>((IObservable<bool>)gameStarter.IsDataLoaded, (Func<bool, bool>)((bool x) => x))));
		disposable = ObservableExtensions.Subscribe<Unit>(Observable.Merge<Unit>(observable, new IObservable<Unit>[1] { Observable.ContinueWith<Unit, Unit>(observable, Observable.AsUnitObservable<User>((IObservable<User>)user.OnLogin)) }), (Action<Unit>)delegate
		{
			SetStatsToDefault();
		});
	}
