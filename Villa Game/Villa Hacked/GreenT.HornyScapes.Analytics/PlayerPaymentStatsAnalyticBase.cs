...
public abstract class PlayerPaymentStatsAnalyticBase : BaseAnalytic<decimal>
{
	...
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<decimal>((IObservable<decimal>)_playerPaymentsStats.OnCohortPaymentStatsUpdate, (Action<decimal>)base.SendEventIfIsValid), (ICollection<IDisposable>)onNewStream);
	}

	public override void SendEventByPass(decimal cohort)
	{
		int num = ConvertToCorrectValue(cohort);
		CohortChangeAmplitudeEvent cohortChangeAmplitudeEvent = new CohortChangeAmplitudeEvent(num);
		_previousCohort = num;
		((IAnalyticSender<AmplitudeEvent>)(object)amplitude).AddEvent((AmplitudeEvent)cohortChangeAmplitudeEvent);
	}
