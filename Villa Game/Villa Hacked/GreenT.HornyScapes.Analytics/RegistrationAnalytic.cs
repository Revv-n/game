...
public class RegistrationAnalytic : BaseAnalytic<Response<UserDataMapper>>, IInitializable
{
	...
	private void SendToAmplitude(Response<UserDataMapper> response)
	{
		...
		((AnalyticsEvent)amplitudeEvent).AddEventParams("registration", (object)response.Data.EmailAdress);
	...
	private void SendToPartner(Response<UserDataMapper> response)
	{
		...
		partnerSendEventStream = ObservableExtensions.Subscribe<long>(Observable.Timer(TimeSpan.FromSeconds(1.0)), (Action<long>)delegate
		{
