...
public class BankAnalytic : BaseEntityAnalytic<Lot>
{
	...
	private void SendBundleLotEvent(ValuableLot<decimal> bundleLot)
	{
		if (bundleLot.Price.Currency.IsInGameCurrency())
		{
			...
			LotBoughAmplitudeEvent lotBoughAmplitudeEvent = new LotBoughAmplitudeEvent(bundleLot);
			((IAnalyticSender<AmplitudeEvent>)(object)amplitude).AddEvent((AmplitudeEvent)lotBoughAmplitudeEvent);
		}
	}
	...
	private void SendSummonEvent(SummonLot summonLot)
	{
		...
		BaseSummonAmplitudeEvent baseSummonAmplitudeEvent = new SummonAmplitudeEvent(summonLot);
		((IAnalyticSender<AmplitudeEvent>)(object)amplitude).AddEvent((AmplitudeEvent)baseSummonAmplitudeEvent);
	}
}
