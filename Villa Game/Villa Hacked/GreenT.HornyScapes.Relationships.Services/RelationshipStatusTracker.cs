...
public class RelationshipStatusTracker : IDisposable
{
	...
	public IObservable<(int RelationshipId, int StatusIndex)> Changed => Observable.AsObservable<(int, int)>((IObservable<(int, int)>)_changed);
	...
	public void Set(Relationship relationship)
	{
		...
		_trackStream = ObservableExtensions.Subscribe<EntityStatus>(Observable.Merge<EntityStatus>(Observable.SelectMany<IReadOnlyList<RewardWithManyConditions>, EntityStatus>(Observable.ToObservable<IReadOnlyList<RewardWithManyConditions>>((IEnumerable<IReadOnlyList<RewardWithManyConditions>>)_relationship.Rewards), (Func<IReadOnlyList<RewardWithManyConditions>, IObservable<EntityStatus>>)((IReadOnlyList<RewardWithManyConditions> x) => (IObservable<EntityStatus>)x.FirstOrDefault().State)), Array.Empty<IObservable<EntityStatus>>()), (Action<EntityStatus>)delegate
		{
			EmitProgressRewardStatus();
		});
	}
