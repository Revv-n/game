...
public sealed class MiniEvent8SlotsLotView : LotView
{
	...
	private void SetPrice(GemShopLot lot)
	{
		...
		if (!flag)
		{
			...
			DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<int>(Observable.Select<int, int>((IObservable<int>)_currencyProcessor.GetCountReactiveProperty(lot.Price.Currency, lot.Price.CompositeIdentificator), (Func<int, int>)((int _) => CurrencyProcessor.IsEnough(cost, lot.Price.CompositeIdentificator) ? 1 : 0)), (Action<int>)_priceColorStatable.Set), (ICollection<IDisposable>)_disposables);
		}
	}
