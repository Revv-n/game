...
public class ItemClock : MonoBehaviour
{
	...
	private Tween ShowTimer()
	{
		...
		return DOTween.Sequence().Join(DOTweenModuleUI.DOFade(canvasGroup, 1f, doScaleDuration)).Append(base.transform.DOScale(Vector3.one, doScaleDuration))
			.Append(base.transform.DOPunchScale(Vector3.one * punchScale, doPunchDuration, 1));
	}

	private Tween HideTimer()
	{
		return DOTween.Sequence().Append(base.transform.DOPunchScale(Vector3.one * punchScale, doPunchDuration, 1)).Insert(doPunchDuration, DOTweenModuleUI.DOFade(canvasGroup, startAlpha, doScaleDuration))
