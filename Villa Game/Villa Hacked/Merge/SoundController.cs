...
public class SoundController : Controller<SoundController>, IAudioPlayer
{
	...
	{
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<float>((IObservable<float>)data.MusicVolume, (Action<float>)delegate
		{
			AtMusicChange();
		}), (ICollection<IDisposable>)audioChangedStream);
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<float>((IObservable<float>)data.SoundVolume, (Action<float>)delegate
		{
			AtSoundChange();
		}), (ICollection<IDisposable>)audioChangedStream);
	...
	private void InnerInit(GameSettings gameSettings, GameStarter gameStarter, IMessengerManager messenger)
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<bool>((IObservable<bool>)gameStarter.IsDataLoaded, (Action<bool>)delegate(bool x)
		{
			Init();
			onGameLoad = x;
		}), (Component)this);
		ObservableExtensions.Subscribe<MessengerUpdateArgs>(Observable.Where<MessengerUpdateArgs>(messenger.OnUpdate, (Func<MessengerUpdateArgs, bool>)GetNewMessage), (Action<MessengerUpdateArgs>)delegate
		{
