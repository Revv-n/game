...
public class GlowTweenMeta : MonoBehaviour, IPoolReturner
{
	...
	public Tween DoEffect()
	{
		...
		sequence.Join(DOTweenModuleUI.DOFade(glow1sr, 1f, 0.1f));
		sequence.Append(glow2sr.transform.DOScale(1.5f, Time));
		sequence.Join(DOTweenModuleUI.DOFade(glow2sr, 1f, Time));
		sequence.Append(DOTweenModuleUI.DOFade(glow1sr, 0f, Time));
		sequence.Append(DOTweenModuleUI.DOFade(glow2sr, 0f, Time));
