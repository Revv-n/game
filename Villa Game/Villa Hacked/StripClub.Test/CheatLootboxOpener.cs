...
public class CheatLootboxOpener : MonoBehaviour
{
	...
	public void OpenLootbox(Lootbox lootbox)
	{
		if (lootbox != null)
		{
			...
			ObservableExtensions.Subscribe<IList<IObservable<SkeletonAnimation>>>(Observable.ToList<IObservable<SkeletonAnimation>>(Observable.Select<int, IObservable<SkeletonAnimation>>(Observable.ToObservable<int>(lootbox.CharacterIdPossibleDrops), (Func<int, IObservable<SkeletonAnimation>>)((int id) => characterSkinLoader.Load(id)))), (Action<IList<IObservable<SkeletonAnimation>>>)delegate
			{
				rewardController.Open(lootbox, CurrencyAmplitudeAnalytic.SourceType.None);
			});
		}
