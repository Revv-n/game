...
public class DisplayRewardsClip : Clip
{
	...
	public void PlayAnim()
	{
		...
		DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<long>(Observable.TakeUntilDisable<long>(Observable.Interval(TimeSpan.FromSeconds(timeBetweenCards + UnityEngine.Random.Range(0f, randomTimeBetweenCards))), (Component)this), (Action<long>)delegate
		{
	...
	private void AnimateAlpha(GameObject card, float duration)
	{
		...
		component.alpha = 0f;
		DOTweenModuleUI.DOFade(component, 1f, duration);
	}
