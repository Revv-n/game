...
public class ShakeCardClip : Clip
{
	...
	private void SetEffects(Gradient gradient)
	{
		...
		((Component)(object)sparks).gameObject.SetActive(value: true);
		MainModule main = sparks.main;
		((MainModule)(ref main)).startColor = MinMaxGradient.op_Implicit(gradient);
	}

	private AnimationState StartAnimation(SkeletonAnimation skeleton)
	{
		base.gameObject.SetActive(value: true);
		skeleton.ClearState();
