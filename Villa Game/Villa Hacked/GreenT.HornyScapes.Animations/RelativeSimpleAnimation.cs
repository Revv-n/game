...
public class RelativeSimpleAnimation : RectTransformAnimation
{
	public override Sequence Play()
	{
		...
		{
			sequence.Append(DOTweenModuleUI.DOAnchorPos(RectTransform, relativePosition, Duration).SetEase(Ease)).Join(RectTransform.DOLocalRotate(relativeRotation, Duration).SetEase(Ease)).Join(RectTransform.DOScale(relativeScale, Duration).SetEase(Ease))
				.Join(DOTweenModuleUI.DOFade(CanvasGroup, relativeAlpha, Duration).SetEase(Ease))
