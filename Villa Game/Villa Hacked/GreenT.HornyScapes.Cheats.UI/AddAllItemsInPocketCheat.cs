...
public class AddAllItemsInPocketCheat : MonoBehaviour, IValidatedCheat, ICheat, IContentSelector, ISelector<ContentType>
{
	...
	{
		Event @event = _eventSettingsProvider.GetEvent(_calendarQueue.GetActiveCalendar(EventStructureType.Event).BalanceId);
		if (@event == null)
		{
			return;
		}
	...
	private List<GIKey> GetEventItemKeys(Event @event)
	{
