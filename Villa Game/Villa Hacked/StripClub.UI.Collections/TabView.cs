...
public class TabView : MonoBehaviour, IView, ISavableState
{
	...
	public void Init(Tab tab)
	{
		...
		hintStream?.Dispose();
		hintStream = DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<ICard>(Observable.Where<ICard>(cards.OnCardUnlock, (Func<ICard, bool>)((ICard _card) => _card.GroupID.Equals(tab.ID))), (Action<ICard>)delegate
		{
