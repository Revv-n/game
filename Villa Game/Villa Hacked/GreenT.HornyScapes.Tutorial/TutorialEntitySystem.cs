...
public abstract class TutorialEntitySystem<TComponent, KStep> : MonoBehaviour where TComponent : BaseTutorialComponent<KStep> where KStep : TutorialStepSO
{
	...
	protected virtual void SubscribeOnComplete()
	{
		...
		foreach (TComponent component in components)
		{
			DisposableExtensions.AddTo<IDisposable>(ObservableExtensions.Subscribe<bool>(Observable.Where<bool>((IObservable<bool>)component.StepModel.IsComplete, (Func<bool, bool>)((bool _value) => _value)), (Action<bool>)delegate
			{
				TryDestroy();
			}), (ICollection<IDisposable>)destroyStream);
		}
	}
