...
public sealed class TutorialToggle<TComponent, KStep> : IDisposable where TComponent : BaseTutorialComponent<KStep> where KStep : TutorialStepSO
{
	...
	public void SubscribeOnActivate(TComponent component)
	{
		...
		{
			onActivateStream.Add(ObservableExtensions.Subscribe<BaseTutorialComponent<KStep>>(component.OnActivate, (Action<BaseTutorialComponent<KStep>>)OnActivate));
		}
	}
	...
	private void OnActivate(BaseTutorialComponent<KStep> step)
	{
		...
		onCompleteStream = ObservableExtensions.Subscribe<bool>(Observable.Where<bool>((IObservable<bool>)step.StepModel.IsComplete, (Func<bool, bool>)((bool _value) => _value)), (Action<bool>)delegate
		{
			OnComplete();
		});
