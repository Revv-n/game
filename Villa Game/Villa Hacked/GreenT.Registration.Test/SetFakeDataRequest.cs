...
public class SetFakeDataRequest : IPostRequest<Response<UserDataMapper>>, IPostRequest<Response<UserDataMapper>, IDictionary<string, string>>
{
	...
	public IObservable<Response<UserDataMapper>> Post(IDictionary<string, string> fields)
	{
		...
		return Observable.Return<Response<UserDataMapper>>(JsonConvert.DeserializeObject<Response<UserDataMapper>>("{\"status\":\"0\", \"error\":\"\", \"data\":{ \"user_name\":\"" + value2 + "\",\"email\":\"" + value3 + "\",\"data\":\"\",\"data_compressed\":0,\"player_id\":\"" + value + "\",\"updated_at\":" + DateTimeOffset.Now.ToUnixTimeSeconds() + " }")).Debug("FAKE SET REQUEST", LogType.Net);
	}
}
