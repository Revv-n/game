...
public class UIParticleSystem : MaskableGraphic
{
	...
	public ParticleSystem.Particle[] particles => _particles;
	...
	protected bool Initialize()
	{
		...
		_particleSystem.GetComponent<ParticleSystemRenderer>().renderMode = _particleSystemRenderMode;
		return true;
	}
	...
	protected override void OnPopulateMesh(VertexHelper vh)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			float num3 = particle.GetCurrentSize(_particleSystem) * 0.5f;
			...
			if (_textureSheetAnimation.enabled)
			{
				...
				num4 = Mathf.Repeat(num4 * (float)_textureSheetAnimation.cycleCount, 1f);
				int num5 = 0;
				...
				{
					num5 = Mathf.FloorToInt(num4 * (float)_textureSheetAnimation.numTilesX);
					int rowIndex = _textureSheetAnimation.rowIndex;
					num5 += rowIndex * _textureSheetAnimation.numTilesX;
					break;
				}
				}
				num5 %= _textureSheetAnimationFrames;
				uv.x = (float)(num5 % _textureSheetAnimation.numTilesX) * _textureSheedAnimationFrameSize.x;
				uv.y = (float)Mathf.FloorToInt(num5 / _textureSheetAnimation.numTilesX) * _textureSheedAnimationFrameSize.y;
