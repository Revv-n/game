...
public class ReactivePropertyWithMultipliableBounds<T> : BoundedReactiveProperty<T>, ISerializable where T : struct, IComparable, IComparable<T>, IEquatable<T>
{
	...
	{
		serializableMax = max;
		multiplierStream = Multiplier.Factor.TakeWhile((double _) => this != null).Subscribe(MultiplyUpperBoundOnFactor);
	}
	...
	{
		if (Add(amount, base.Min).CompareTo(base.Value) > 0)
		{
			return false;
		}
		return true;
	...
	{
		base.Value = ExtensionMethods.Add(base.Value, amount);
	}
