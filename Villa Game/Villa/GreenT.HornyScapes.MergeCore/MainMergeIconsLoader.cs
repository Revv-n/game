...
public class MainMergeIconsLoader : ILoader<IEnumerable<Sprite>>
{
	...
	{
		return Observable.Start(() => GetPathByBundleType(projectSettings, bundleType), Scheduler.MainThread).ContinueWith(assetBundlesLoader.DownloadAssetBundle).Select(GetObjectsFromAssetBundleSubFolder<Sprite>);
	}
