...
public class GameItemController : Controller<GameItemController>, ISavableState, IMasterController
{
	...
	public IObservable<Unit> StartFieldCreat => _startFieldCreat;

	public IObservable<Unit> EndFieldCreat => _endFieldCreat;
	...
	public ReadOnlyReactiveProperty<ContentType> CurrentContentType => currentContentType.ToReadOnlyReactiveProperty();
	...
	public override void Preload()
	{
		...
		disposable?.Dispose();
		disposable = (from mergeField in gameStarter.IsGameActive.FirstOrDefault((bool x) => x).SelectMany(fieldProvider.OnNew).TakeUntil((from x in gameStarter.IsGameActive.SkipLatestValueOnSubscribe()
				where !x
				select x).Take(1))
			where mergeField.Type == ContentType.Event
			select mergeField).Subscribe(delegate(MergeField mergeField)
		{
	...
	private Queue<GIData> GetPocketByField(ContentType fieldType)
	{
		if (fieldType != 0)
		{
			return eventProvider.CurrentCalendarProperty.Value.@event.Data.PocketRepository.Queue;
		}
