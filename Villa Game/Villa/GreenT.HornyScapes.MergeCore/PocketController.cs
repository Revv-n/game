...
public class PocketController : Controller<PocketController>, ISavableState
{
	...
	public IObservable<GIData> OnItemAdd => onItemAdd.AsObservable();

	public IObservable<PocketMemento> OnClear => onClear.AsObservable();

	public IObservable<GIData> OnItemRemove => onItemRemove.AsObservable();
	...
	private Queue<GIData> GetItemQueue(ContentType contentType)
	{
		if (contentType != 0)
		{
			return _provider.CurrentCalendarProperty.Value.@event.Data.PocketRepository.Queue;
		}
	...
	{
		Queue<GIData> queue = _provider.CurrentCalendarProperty.Value.@event.Data.PocketRepository.Queue;
		queue.Clear();
		onClear.OnNext(pocketData);
		foreach (GIData item in newCollection)
		{
	...
	{
		_provider.CurrentCalendarProperty.Value.@event.Data.PocketRepository.DeleteItem(itemToRemove);
	}
	...
	private bool TryGetQueue(out Queue<GIData> queue)
	{
		...
		{
			queue = _provider.CurrentCalendarProperty.Value.@event.Data.PocketRepository.Queue;
		}
		return queue != null;
	}
	...
	public void AtButtonClick()
	{
		...
		{
			GIData value = queue.Dequeue().Copy().SetCoordinates(pnt);
			onItemRemove.OnNext(value);
		}
	}
