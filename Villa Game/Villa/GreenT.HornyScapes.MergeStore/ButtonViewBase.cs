...
public abstract class ButtonViewBase<T> : MonoView<T>
{
	...
	{
		_buyButton.onClick.AsObservable().Subscribe(delegate
		{
			TryEmit();
		}).AddTo(_mainStream);
	}
	...
	{
		_currencyProcessor.GetChangeStream(GetCurrencyType(), OnCurrencyChange).AddTo(_mainStream);
	}
