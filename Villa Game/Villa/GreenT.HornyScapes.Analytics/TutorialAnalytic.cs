...
public class TutorialAnalytic : BaseEntityAnalytic<TutorialStep>, ISavableState
{
	...
	public override void Track()
	{
		...
		if (steps.Count != 0)
		{
			...
			OnCompleteStepObservable().Subscribe(base.SendEventIfIsValid).AddTo(disposables);
	...
	{
		return steps.Where((TutorialStep _step) => !itemsStreams.ContainsKey(_step.StepID) && !_step.IsComplete.Value).Select(EmitStepOnComplete).Merge();
	}

	private IObservable<TutorialStep> EmitStepOnComplete(TutorialStep _step)
	{
		return (from _value in _step.IsComplete
			where _value
	...
	private void SendToAmplitude(TutorialStep entity)
	{
		...
		amplitudeEvent.AddEventParams("tutorial_step", entity.StepID);
