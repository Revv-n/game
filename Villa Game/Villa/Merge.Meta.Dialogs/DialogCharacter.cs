...
public class DialogCharacter : MonoBehaviour
{
	...
	public Tween DoVisible(bool visible)
	{
		...
		s.Join(CharTr.DOAnchorPos(anchors.GetPosition(visible), visibleTime));
	...
	public Tween DoSwap(Action swap)
	{
		...
		Sequence s = DOTween.Sequence();
		s.Join(CharTr.DOAnchorPos(anchors.GetPosition(visible: false), visibleTime));
		s.Join(DoBig(isBig: false));
		s.AppendCallback(delegate
		{
			swap();
		});
		s.Append(CharTr.DOAnchorPos(anchors.GetPosition(visible: true), visibleTime));
		visibleTween = s;
		return visibleTween;
	}
