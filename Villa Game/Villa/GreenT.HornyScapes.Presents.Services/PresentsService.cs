...
public class PresentsService : IInitializable, IDisposable
{
	...
	public IObservable<PresentView> Spended => _spended;

	public IObservable<(PresentView PresentView, PresentSpendFailReason Reason)> SpendFailed => _spendFailed;
	...
	{
		_currencyStream = _gameStarter.IsGameActive.Where((bool x) => x).Subscribe(delegate
		{
			SetCurrencies();
		});
	}
	...
	public void Set(CharacterInfo characterInfo, Relationship relationship)
	{
		...
		_disposables?.Clear();
		_presentsViewTapTracker.PresentSended.Subscribe(TrySendPresent).AddTo(_disposables);
		_presentsViewTapTracker.PressStarted.Subscribe(StartTrackPresent).AddTo(_disposables);
		_presentsViewTapTracker.PressEnded.Subscribe(delegate(PresentView presentView)
		{
