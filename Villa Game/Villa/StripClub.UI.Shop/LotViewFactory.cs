...
public class LotViewFactory : MonoBehaviour, IFactory<Lot, LotView>, IFactory
{
	...
	public virtual LotView Create(Lot lot)
	{
		...
		try
		{
			LotView prefab = viewPrefabs.First((LotView _view) => _view.GetType().Equals(viewType));
			LotView lotView = container.InstantiatePrefabForComponent<LotView>(prefab, viewContainer);
			lotView.Set(lot);
			return lotView;
		}
