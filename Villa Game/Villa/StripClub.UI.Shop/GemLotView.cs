...
public class GemLotView : LotView
{
	...
	public override void Set(Lot lot)
	{
		...
		_infoDisposable = _localizationService.ObservableText(lot.LocalizationKey).Subscribe(delegate(string text)
		{
			lotInfo.text = text;
		});
	...
	public override void Purchase()
	{
		...
		onBuyStream?.Clear();
		_isPurchasing = true;
		purchaser.OnResult.Subscribe(delegate(bool result)
		{
	...
	private void SetStickers(GemShopLot lot)
	{
		...
		{
			_stickerDisposable = _localizationService.ObservableText(text2).Subscribe(delegate(string text)
			{
