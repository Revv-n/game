...
public class BankTabView : MonoView<BankTab>
{
	...
	public override void Set(BankTab tab)
	{
		...
		IReadOnlyReactiveProperty<string> source = _localizationService.ObservableText(base.Source.LocalizationKey);
		...
		foreach (TextMeshProUGUI title in array)
		{
			...
			source.Subscribe(delegate(string value)
			{
				title.text = value;
	...
	private void GetAvailableSectionLot()
	{
		...
		if (base.Source.ID == 1001)
		{
			...
			sectionAvailableLots.First()?.Locker.IsOpen.Subscribe(delegate(bool isOpen)
			{
				if (isOpen)
				{
					TabMark = EvaluateMarks();
		...
		{
			lotStreams.Add(lot.OnLotReceived().Subscribe(delegate
			{
				TabMark = EvaluateMarks();
				SetMarks(TabMark);
			}));
