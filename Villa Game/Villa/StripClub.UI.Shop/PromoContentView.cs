	...
	public void Set(List<LinkedContent> content)
	{
		...
		if (content.Any() && content != null)
		{
			...
			for (int i = 1; i < content.Count; i++)
			{
				...
				PromoCardView prefab = ((linkedContent2 is SkinLinkedContent || linkedContent2 is CardLinkedContent) ? _girlSmallCardTemplate : _anySmallCardTemplate);
				_container.InstantiatePrefab(prefab, _smallCardsRoot).GetComponent<PromoCardView>().Set(linkedContent2);
			}
