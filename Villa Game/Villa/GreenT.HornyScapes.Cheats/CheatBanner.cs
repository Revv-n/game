...
public class CheatBanner : MonoBehaviour
{
	...
	private void Start()
	{
		...
		(from _ in AddContractsButton.onClick.AsObservable()
			select InputField.text).Subscribe(AddContracts);
		GenerateReportButton.onClick.AsObservable().Subscribe(delegate
		{
			GenerateReport();
		});
	}
