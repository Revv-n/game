...
public class CheatEvent : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		_calendarInProgressStream = _calendarQueue.OnCalendarActiveNotNull().Where(delegate(CalendarModel model)
		{
			EventStructureType eventType = model.EventType;
			return eventType == EventStructureType.Event || eventType == EventStructureType.Mini;
		}).SelectMany((CalendarModel _) => _eventSettingsProvider.GetActiveModels(_calendarQueue.ToList()))
			.Subscribe(SetSprites);
	}

	private void SetSprites(CalendarModel calendarModel)
	{
		GreenT.HornyScapes.Events.Event @event = _eventSettingsProvider.GetEvent(calendarModel.BalanceId);
		if (@event != null)
		{
