...
public class NicknameWindow : PopupWindow
{
	...
	{
		playButton.Button.OnClickAsObservable().TakeUntilDisable(this).Subscribe(SetNickName)
			.AddTo(this);
	}
	...
	private void OnSetResponse(Response<UserDataMapper> response)
	{
		...
		IErrorMessage errorMessage = errorMessageFactory.Create("ui.error.nickname_setup.", response.Status);
