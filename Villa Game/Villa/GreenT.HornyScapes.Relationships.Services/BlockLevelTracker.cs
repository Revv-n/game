...
public sealed class BlockLevelTracker
{
	...
	public void AddBlockedReward(int relationshipId, BaseRewardView rewardView)
	{
		...
		IObservable<EntityStatus> source = state.DistinctUntilChanged();
		IDisposable subscription = null;
		subscription = source.Subscribe(delegate(EntityStatus status)
		{
	...
	public void TryStartAnimation(int relationshipId)
	{
		...
		{
			Observable.Timer(TimeSpan.FromSeconds(_delay)).Subscribe(delegate
			{
