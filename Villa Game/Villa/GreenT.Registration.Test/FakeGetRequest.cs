...
public class FakeGetRequest : IPostRequest<Response<UserDataMapper>>, IPostRequest<Response<UserDataMapper>, IDictionary<string, string>>
{
	public IObservable<Response<UserDataMapper>> Post(IDictionary<string, string> fields)
	{
		...
		return Observable.Return(JsonConvert.DeserializeObject<Response<UserDataMapper>>("{\"status\":\"0\", \"error\":\"\", \"data\":{ \"user_name\":\"Fake Login\",\"email\":\"" + value + "\",\"data\":\"\",\"data_compressed\":0,\"player_id\":\"" + value2 + "\",\"updated_at\":" + DateTimeOffset.Now.ToUnixTimeSeconds() + " }")).Debug("FAKE GET RESPONSE", LogType.Net);
	}
}
