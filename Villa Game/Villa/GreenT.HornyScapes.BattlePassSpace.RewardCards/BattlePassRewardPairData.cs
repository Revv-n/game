...
public class BattlePassRewardPairData
{
	...
	public BattlePassRewardViewType GetViewType()
	{
		...
		if (!FreeReward.IsEmpty() && !PremiumReward.IsEmpty() && PremiumReward.First() == FreeReward.First())
		{
	...
	{
		if (PremiumReward.IsEmpty() && TryGetState(FreeReward, out var targetState))
		{
			return targetState;
		}
		if (FreeReward.IsEmpty() && TryGetState(PremiumReward, out targetState))
		{
			return targetState;
		}
