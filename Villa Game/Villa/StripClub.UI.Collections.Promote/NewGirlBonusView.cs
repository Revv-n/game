...
public class NewGirlBonusView : MonoBehaviour
{
	...
	public void Init(CharacterMultiplierBonus bonus)
	{
		...
		_nameDisposable = _localization.ObservableText(bonus.SpawnerName).Subscribe(delegate(string text)
		{
			spawnerName.text = char.ToUpper(text[0]) + text.Substring(1);
		});
		_bonusDisposable = _localization.ObservableText(bonus.Name).Subscribe(delegate(string text)
		{
			string input = text;
