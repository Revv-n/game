...
public class CharacterMessageView : MessageView<CharacterChatMessage>
{
	...
	public override void Set(CharacterChatMessage message)
	{
		...
		if (authorText != null)
		{
			...
			_authorDisposable = _localizationService.ObservableText(character.NameKey).Subscribe(delegate(string text)
			{
				authorText.text = text;
			});
		}
		_messageDisposable = _localizationService.ObservableText(message.LocalizationKey).Subscribe(delegate(string text)
		{
		...
		galleryController.GetOrLoadMedia(message.MediaID.Value).Subscribe(delegate(Media _media)
		{
		...
		galleryController.GetOrLoadMedia(message.MediaID.Value).Do(delegate(Media _media)
		{
			mediaThumbnail.sprite = _media.Thumbnail;
			OnButtonClick = delegate
			{
	...
	public void SetMessageAsRead(CharacterChatMessage chatMessage)
	{
		if (!chatMessage.State.Contains(ChatMessage.MessageState.Read))
		{
			...
			delayStreamToMarkAsRead = Observable.Timer(TimeSpan.FromSeconds(secondsToMarkMessageAsRead)).TakeUntilDisable(this).Subscribe(delegate
			{
