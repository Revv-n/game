...
public sealed class SaveProviderFacade : IDisposable
{
	...
	public IObservable<SavedData> Deserialize<T>() where T : SavedData, new()
	{
		...
		{
			return localSaveProvider.LoadSave().CombineLatest(remoteSaveProvider.LoadSave(), (SavedData local, SavedData remote) => (local: local, remote: remote)).Select(CompareSaveDate);
		}
