...
public class Currencies : ISerializable
{
	...
	public IObservable<(CurrencyType, SimpleCurrency)> OnNewCurrency => _newCurrency;
	...
	public bool TryAdd(CurrencyType currencyType, int value, CompositeIdentificator identificator = default(CompositeIdentificator))
	{
		...
		{
			OldCurrenciesDict.FirstOrDefault((SimpleCurrency value) => value.UniqueKey() == CurrenciesDict[_commonCurrencyKey].GetOldUniqueKey() && value.Identificator == CurrenciesDict[_commonCurrencyKey].Identificator).Count.Value += value;
		}
		CurrenciesDict[_commonCurrencyKey].Count.Value += value;
		return true;
	}
