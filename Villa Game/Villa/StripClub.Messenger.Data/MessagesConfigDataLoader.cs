...
public class MessagesConfigDataLoader : ILoader<IEnumerable<int>, IEnumerable<MessageConfigMapper>>
{
	...
	public IObservable<IEnumerable<MessageConfigMapper>> Load(IEnumerable<int> dialogueIDCollection)
	{
		...
		if (!source.Any())
		{
			return Observable.Return(cachedMappers);
		}
		return (from _ in characterMessagesLoader.Load().Debug(GetType().Name + ": Character messages mappers loading", LogType.Data | LogType.Messenger).Zip(playerMessagesLoader.Load().Debug(GetType().Name + ": Player messages mappers loading", LogType.Data | LogType.Messenger), (IEnumerable<CharacterMessageConfigMapper> character, IEnumerable<PlayerMessageConfigMapper> player) => character.Cast<MessageConfigMapper>().Union(player))
				.Do(SaveInCache)
			select cachedMappers.ToArray()).Debug(GetType().Name + ": Messenger messages mappers loading", LogType.Data | LogType.Messenger);
	}
