...
public class RelationshipUiSetter : MonoView<(GreenT.HornyScapes.Characters.CharacterInfo Character, Relationship Relationship)>, IDisposable
{
	...
	public IObservable<IReadOnlyList<BaseRewardView>> RewardsShowed => _rewardShowed.AsObservable();
	...
	public override void Set((GreenT.HornyScapes.Characters.CharacterInfo Character, Relationship Relationship) source)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			baseRewardView.TooltipRequested.Subscribe(ShowTooltip).AddTo(this);
			baseRewardView.ProgressChanged.Subscribe(ScrollTo).AddTo(this);
			baseRewardView.RewardClaimed.Subscribe(item.ClaimReward).AddTo(this);
		...
		visibleViews.First((BaseRewardView x) => x.Source.Rewards == rewardsArray[^1]).SetAsLastInTrack();
		_activeRewardTracker.Set(item);
		_presentStream?.Dispose();
		_presentStream = _presentsService.Spended.Subscribe(OnPresentSpended);
		_presentStream = _presentsService.SpendFailed.Subscribe(OnPresentSpendFailed);
	...
	private void Activate()
	{
		...
		if (characterInfo != null || relationship != null)
		{
			...
			_rewardShowed?.OnNext(_rewardViewProvider.VisibleViews);
		}
	}
