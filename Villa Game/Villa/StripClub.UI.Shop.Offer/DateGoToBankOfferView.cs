...
public class DateGoToBankOfferView : LotView
{
	...
	public override void Set(Lot lot)
	{
		...
		string[] array = _localizationService.Text(bundleLot.DescriptionKey).Split('|');
		if (array.Length == 3)
		{
	...
	private void SetupPresetData(BundleLot shopBundleLot)
	{
		...
		{
			Object.Destroy(_offerBackground.gameObject);
		}
		GameObject obj = shopBundleLot.Settings.GetBackground();
		_ = obj == null;
		if (Object.Instantiate(obj, _backgroundHolder).TryGetComponent<Background>(out _offerBackground) && shopBundleLot.Settings.IsCharacterSpriteOverridden)
		{
