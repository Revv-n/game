...
public class HouseBackgroundBuilder : MonoBehaviour
{
	...
	public IObservable<Bounds> HouseBounds => onBoundsChanged.ToReadOnlyReactiveProperty(houseBounds);
	...
	private void GameStarterSubscribe()
	{
		...
		{
			Observable.Timer(TimeSpan.FromSeconds(20.0)).Subscribe(delegate
			{
				ReloadTiles();
			}).AddTo(this);
			return;
		}
		gameStarter.IsDataLoaded.Subscribe(delegate(bool state)
		{
			if (state)
			{
				Observable.Timer(TimeSpan.FromSeconds(20.0)).Subscribe(delegate
	...
	{
		backgroundSpritesManager.ObserveAdd().Subscribe(delegate(DictionaryAddEvent<int, Sprite> _pair)
		{
	...
	{
		cellSize = backgroundSpritesManager.First().Value.bounds.size;
		foreach (KeyValuePair<int, Sprite> item in backgroundSpritesManager)
		{
	...
	private void Build()
	{
		...
		int num2 = 0;
		while (num2 != num && num2 != backgroundSpritesManager.Count)
		{
			SpriteRenderer spriteRenderer = CreateTile(backgroundSpritesManager[num2], new Vector2(x, y));
			tiles.Add(spriteRenderer);
			houseBounds.Encapsulate(spriteRenderer.bounds);
			onBoundsChanged.OnNext(houseBounds);
	...
	{
		backgroundLoader.Load().Do(backgroundSpritesManager.ReplaceRange).ContinueWith(backgroundPreloader.Load())
			.Do(backgroundSpritesManager.ReplaceRange)
			.DoOnCompleted(ReplaceTiles)
			.Subscribe(delegate
			{
			})
			.AddTo(this);
