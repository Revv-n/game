...
public class RelationshipProgressView : MonoView, IDisposable
{
	...
	public IObservable<BaseRewardView> ProgressChanged => _progressChanged.AsObservable();
	...
	{
		ReactiveProperty<int> reactiveProperty = _currencies.Get(_pointsType, _id);
		SetProgress(reactiveProperty.Value, immediate: true);
		_trackStream = reactiveProperty.Subscribe(delegate(int value)
		{
			SetProgress(value);
		});
	}
