...
public sealed class RelationshipsToolTipOpener : MonoView<RelationshipsToolTip>, IPointerClickHandler, IEventSystemHandler
{
	...
	public IObservable<RelationshipsToolTipOpener> Requested => _requested.AsObservable();
	...
	public void ShowToolTip(RelationshipsToolTip toolTip, RectTransform parent, int count)
	{
		if (!IsPlaying.Value)
		{
			...
			_timerDisposable = ShutDownObservable().DoOnCancel(HideToolTip).Subscribe(delegate
			{
				HideToolTip();
			});
		}
	...
	{
		return _pointerPress.OnPointerClick().Skip(1).TakeUntil(IsPlaying.First((bool x) => !x))
			.AsUnitObservable()
			.Take(1);
	}
}
