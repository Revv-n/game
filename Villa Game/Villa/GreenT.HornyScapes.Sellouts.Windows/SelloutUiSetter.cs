...
public sealed class SelloutUiSetter : MonoView<Sellout>
{
	...
	public override void Set(Sellout sellout)
	{
		...
		for (int i = 0; i < rewardBlocks.Count; i++)
		{
			...
			selloutRewardView.ProgressChanged.Subscribe(delegate
			{
				_scrollSnapController.SnapToAppropriateReward();
			}).AddTo(_disposables);
			selloutRewardView.RewardClaimed.Subscribe(delegate
			{
				_scrollSnapController.SnapToAppropriateReward();
			}).AddTo(_disposables);
		...
		_trackerStream = _selloutRewardsTracker.ActiveRewardsChanged.Subscribe(CheckReward);
		sellout.CanPointsTrack.Subscribe(delegate(bool canTrack)
		{
	...
	{
		_tutorialButton.OnClickAsObservable().TakeUntilDisable(this).Subscribe(delegate
		{
			OpenTutorial();
		})
			.AddTo(_disposables);
		_bankButton.OnClickAsObservable().TakeUntilDisable(this).Subscribe(delegate
		{
			OpenBank();
		})
			.AddTo(_disposables);
