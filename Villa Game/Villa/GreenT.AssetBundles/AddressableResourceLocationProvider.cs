...
public class AddressableResourceLocationProvider
{
	public IObservable<IList<IResourceLocation>> GetLocations(string groupName)
	{
		...
		return Observable.FromCoroutine((IObserver<IList<IResourceLocation>> observer, CancellationToken cancellationToken) => GetLocationsCoroutine(locationsHandler, observer, cancellationToken));
	}
