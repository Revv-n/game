...
public class LocalizedTextMeshPro : MonoBehaviour
{
	...
	{
		localizationStream?.Dispose();
		IObservable<string> source = _localizationService.ObservableText(key);
		if (arguments.Length != 0)
		{
			TryLocalizeArguments(arguments);
			source = source.Select((string x) => string.Format(x, arguments));
		}
		this.arguments = arguments;
		localizationStream = source.Subscribe(SetText);
	}
