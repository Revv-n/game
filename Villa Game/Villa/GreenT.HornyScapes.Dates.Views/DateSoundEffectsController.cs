...
public sealed class DateSoundEffectsController : IDisposable
{
	...
	private void StartSoundtracks(DatesSoundtracksInfo soundtracksInfo)
	{
		...
		foreach (DatesSoundtracks datesSoundtracks in soundtracks)
		{
			...
			IDisposable value = CreateTrackSequence(datesSoundtracks, soundtracksInfo, subscriptionId).Subscribe(delegate
			{
	...
	private IObservable<Unit> CreateTrackSequence(DatesSoundtracks soundtrack, DatesSoundtracksInfo parentSoundtracksInfo, string subscriptionId)
	{
		return Observable.Defer(delegate
		{
			...
			IObservable<Unit> observable = ((0f < startCooldown) ? Observable.FromCoroutine((IObserver<Unit> observer) => StartDelayCoroutine(startCooldown, observer)) : Observable.Empty<Unit>());
			...
			IObservable<Unit> first = Observable.FromCoroutine((IObserver<Unit> observer) => PlaySoundCoroutine(firstSound, withFadeIn: true, parentSoundtracksInfo.FadeIn, observer, parentSoundtracksInfo, callerContextFirst));
			IObservable<Unit> observable2 = source.Aggregate(Observable.Empty<Unit>(), (IObservable<Unit> chain, DatesSound dateSound) => chain.Concat(Observable.FromCoroutine((IObserver<Unit> observer) => PlaySoundCoroutine(dateSound, withFadeIn: false, 0f, observer, parentSoundtracksInfo, callerContextRest))));
			IObservable<Unit> observable3 = first.Concat(observable2);
			return (!soundtrack.IsRepeat) ? observable.Concat(observable3) : observable.Concat(observable3.Repeat());
		});
	}
	...
	public void Stop(bool immediate = false)
	{
		...
		if (_currentSoundtracksInfo != null)
		{
			...
			_fadeOutAllStream = FadeOutAll((_currentSoundtracksInfo != null) ? _currentSoundtracksInfo.FadeOut : 0f).Subscribe(delegate
			{
			}, delegate(Exception exception)
			{
				Debug.LogException(exception);
	...
	{
		return Observable.FromCoroutine((IObserver<Unit> observer) => FadeOutCoroutine(fadeOutDuration, observer));
	}
