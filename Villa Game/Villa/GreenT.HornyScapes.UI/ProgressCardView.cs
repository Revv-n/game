...
public class ProgressCardView : GeneralCardView
{
	...
	protected void TrackPromote()
	{
		...
		if (promote != null)
		{
			promote.Level.TakeUntilDisable(this).Subscribe(OnUpdateLevel).AddTo(promoteLvlStream);
			promote.Progress.Skip(1).TakeUntilDisable(this).Subscribe(UpdateProgressBar)
				.AddTo(promoteLvlStream);
		}
	}
