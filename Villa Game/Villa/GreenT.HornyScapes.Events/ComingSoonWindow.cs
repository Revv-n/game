...
public class ComingSoonWindow : PopupWindow
{
	...
	public void Set(CalendarModel calendarModel)
	{
		...
		changeStateStream = (from _ in calendarModel.CalendarState.Where((EntityStatus _state) => _state != EntityStatus.Blocked).Take(1)
			where IsOpened
			select _).Subscribe(OnChangeSourceState);
