...
public sealed class MiniEvent : IRewardHolder, IIdentifiable, IDisposable, ISavableState
{
	...
	public MiniEvent(int calendarId, int position, int balanceId, int eventId, bool isMultiTabbed, IEnumerable<IController> controllers, CompositeIdentificator currencyIdentificator, ViewSettings? viewSettings, PromoSettings promoSettings, ConfigType configType)
	{
		PriorityId = position;
