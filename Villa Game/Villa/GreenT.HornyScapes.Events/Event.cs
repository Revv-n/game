...
public class Event : IBundleProvider<EventBundleData>, IRewardHolder, ISavableState, IDisposable, IHaveTrackingRewards
{
	...
	{
		IObservable<NotifyReward> source = _eventRewards.Select((EventReward _reward) => _reward.OnUpdate).Merge();
		_anyRewardUpdateStream = source.Subscribe(_onRewardUpdate.OnNext);
	}
