...
public class MonetizationPaymentConnector : MonoBehaviour, IPaymentConnector
{
	...
	{
		_signalBus.Subscribe<CancelPaymentSignal>(OnPaymentCancelled);
	}
	...
	private void ShowChoosePaymentType(int monetizationID, string itemName, string price, string imageUrl)
	{
		...
		_getImageSubscription = HttpRequestExecutor.GetSprite(imageUrl).Subscribe((Action<Sprite>)delegate(Sprite sprite)
		{
	...
	private void ShowPopup(int monetizationID, string itemName, string price, Sprite imageUrl)
	{
		...
		_paymentSelectedSubscription = typePyamentPopup.OnPaymentSelected.Subscribe(delegate(PaymentMethod method)
		{
			switch (method)
			{
		...
		_exitClickedSubscription = typePyamentPopup.OnExitClicked.Subscribe(delegate
		{
	...
	private void StartPurchase(int monetizationID, string itemName, string price, string typePayment)
	{
		...
		_paymentRequestSubscription = _requester.GetPaymentPage(typePayment, _dataStorage.GetString("player_id"), monetizationID).Subscribe((Action<PaymentResponse>)delegate(PaymentResponse response)
		{
	...
	private void OpenWebView(PaymentResponse response)
	{
		...
		_webViewSubscription = _webView.OnCloseWebViewObservable.Subscribe(delegate
		{
	...
	private void WebViewClosed()
	{
		...
		int retryCount = 0;
		_watchPaymentSubscription = Observable.Interval(TimeSpan.FromSeconds(5.0)).StartWith(0L).SelectMany((long _) => _invoiceStatusRequester.GetInvoiceStatus(_paymentId))
			.Subscribe(delegate(Response<PaymentIntentData> response)
			{
