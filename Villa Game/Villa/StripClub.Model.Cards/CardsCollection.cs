...
public sealed class CardsCollection : IDisposable
{
	...
	public IObservable<ICard> OnNewCard => newCard;

	public IObservable<ICard> OnNewPromote => newPromote;
	...
	public IObservable<ICard> OnCardUnlock => unlockedCard;

	public IObservable<(ICard, int)> OnNewSoulsCard => newSoulsCard;
	...
	public CardsCollection(IFactory<ICard, IPromote> promoteFactory)
	{
		...
		Assert.IsNotNull(promoteFactory);
	...
	public void Connect(IPromote promote, ICard card, bool callUnlock = false)
	{
		...
		promote.Level.DoOnError(delegate
		{
		}).Subscribe(card.Bonus.SetLevel).AddTo(disposables);
		card.Bonus.Apply();
	...
	public IReadOnlyReactiveProperty<bool> AnyIsNew()
	{
		...
		return (from _list in enumerable.CombineLatest()
			select _list.Any((bool x) => x)).ToReactiveProperty(enumerable.Any((IReactiveProperty<bool> _isNew) => _isNew.Value));
	}
