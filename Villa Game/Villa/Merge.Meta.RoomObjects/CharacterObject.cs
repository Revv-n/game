...
public class CharacterObject : GameRoomObject<CharacterObjectConfig>
{
	...
	public override IObservable<Bounds> GetBounds()
	{
		...
		{
			return from _ in DownloadAndSetSkin(characterID, 0)
				select animationMeshRenderer.bounds;
		}
	...
	private IObservable<(CharacterAnimationSettings, SkeletonAnimation)> DownloadAndSetSkin(int skinVisualId, int skinSettingsId)
	{
		...
		animationLoadObservable = base.Config.GetSkinSettings(skinSettingsId).SelectMany((CharacterAnimationSettings skin) => from animation in skinLoader.Load(skinVisualId)
			select (skin: skin, animation: animation)).Do(delegate((CharacterAnimationSettings skin, SkeletonAnimation animation) pair)
		{
		...
		}, delegate(Exception ex)
		{
			...
			.Share();
		animationLoadStream = animationLoadObservable.Subscribe().AddTo(this);
		return animationLoadObservable;
	}
	...
	public override void SetVisible(bool visible)
	{
		...
		void SoundOnClick()
		{
			...
			{
				clickStream = collideTracker.OnClick.Subscribe(delegate
				{
					OnClick();
				});
			}
