...
public abstract class RouletteBaseFactory<T, P> : IFactory<T, P>, IFactory where T : RouletteSummonMapper where P : RouletteSummonLot
{
	...
	public P Create(T mapper)
	{
		...
		LinkedContentAnalyticData param = _analyticDataFactory.Create(CurrencyAmplitudeAnalytic.SourceType.Roulette);
		LinkedContent defaultReward = _lootboxContentFactory.Create(mapper.reward, param);
		LinkedContent uniqueReward = _lootboxContentFactory.Create(mapper.main_reward, param);
