...
public class RewardContainer : MonoView
{
	...
	[SerializeField]
	private GreenT.HornyScapes.Animations.Animation zoomInOut;
	...
	{
		controller.OnClear.Where((GreenT.HornyScapes.MergeCore.PocketController.PocketMemento _) => contentSelectorGroup.Current == contentType).Subscribe(delegate
		{
			bool viewActive = controller.NotEmpty();
			SetViewActive(viewActive);
		}).AddTo(this);
		controller.OnItemAdd.Where((GIData _) => contentSelectorGroup.Current == contentType).Subscribe(delegate
		{
			bool flag = controller.NotEmpty();
			SetViewActive(flag);
			if (flag)
		...
		controller.OnItemRemove.Where((GIData _) => contentSelectorGroup.Current == contentType).Subscribe(delegate(GIData _item)
		{
