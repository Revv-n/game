...
public abstract class LastChanceButtonView : MonoView
{
	...
	private void Init(LastChanceController lastChanceController, TimeHelper timeHelper, [InjectOptional] TimeInstaller.TimerCollection timers, IClock clock)
	{
		...
		_timers = timers;
		_clock = clock;
		_lastChanceActivatedDisposable = _lastChanceController.OnLastChanceActivated.Subscribe(OnLastChanceActivated);
		_lastChanceDeactivatedDisposable = _lastChanceController.OnLastChanceDeactivated.Subscribe(OnLastChanceDeactivated);
