...
public class ToolTipOpenerGirlZoom : GirlToolTipOpener
{
	...
	private void StartZoomToolTip()
	{
		...
		IObservable<Unit> observable = base.transform.OnTransformChildrenChangedAsObservable().First().Share();
		observableDisposable = observable.Subscribe(onChildChanged);
		zoomDisposable = _navigation.OnZoom().Delay(TimeSpan.FromSeconds(zoomDelay)).SkipUntil(observable)
			.Subscribe(onZoomCallback)
			.AddTo(this);
	}
