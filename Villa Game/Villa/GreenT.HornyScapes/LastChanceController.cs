...
public sealed class LastChanceController : IDisposable
{
	...
	public IObservable<LastChance> OnLastChanceActivated => _onLastChanceActivated.AsObservable();

	public IObservable<LastChance> OnLastChanceDeactivated => _onLastChanceDeactivated.AsObservable();
	...
	public void Init()
	{
		_timer = Observable.Interval(TimeSpan.FromSeconds(1.0)).Subscribe(delegate
		{
			HandleLastChances();
		});
	}
