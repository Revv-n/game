...
public class PromocodeService
{
	...
	public IObservable<string> OnResponseMessage => responseMessageSubject;
	...
	public void TryToActivatePromocode(string promocodeName)
	{
		...
		wait?.Dispose();
		wait = (from _ in Observable.EveryUpdate().SkipUntil(Observable.Timer(TimeSpan.FromSeconds(0.5)))
			where popupWindow.IsOpened
			select _).Take(1).Subscribe(delegate
		{
			promocodeActivationDisposable?.Dispose();
			promocodeActivationDisposable = ActivatePromocode(promocodeName).Subscribe(PromocodeResponse);
	...
	public IObservable<PromocodeRewardResponse> ActivatePromocode(string promocodeName)
	{
		...
		return promocodeActivationRequest.Post(promocodeName, platformName).Catch(delegate(Exception ex)
		{
	...
	{
		showRewardStream = Observable.EveryUpdate().SkipUntil(Observable.Timer(TimeSpan.FromSeconds(0.5))).Take(1)
			.Subscribe(delegate
			{
	...
	{
		hidePromocodeWindowStream = (from _ in Observable.EveryUpdate().SkipUntil(Observable.Timer(TimeSpan.FromSeconds(waitTime)))
			where popupWindow.IsOpened || forcedClose
			select _).Take(1).Subscribe(delegate
		{
			popupWindow?.Close();
		});
