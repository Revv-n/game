...
public sealed class ScrollPool : MonoView
{
	...
	public IObservable<RatingPlayerView> OnOpponentViewPositionUpdate => _onOpponentViewPositionUpdate;
	...
	{
		_disposable?.Dispose();
		_disposable = _scrollRect.OnValueChangedAsObservable().Subscribe(delegate
		{
			OnValueChanged();
		});
	...
	private void TryMoveToTop(int newIndex)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			_currentPlayerParent.SwapVisibleState(isActive: false);
			_playerViewTrackDisposable = _currentPlayerParent.transform.ObserveEveryValueChanged((Transform value) => value.position.y).Subscribe(delegate(float currentYPosition)
			{
	...
	{
		_playerView.transform.ObserveEveryValueChanged((Transform value) => value.position.y).Subscribe(delegate(float currentYPosition)
		{
