...
public sealed class SelloutPointsView : MonoView
{
	...
	private void Set(Sellout sellout)
	{
		...
		_sellout.CanPointsTrack.Where((bool result) => !result).Subscribe(delegate
		{
			Deactivate();
		}).AddTo(_subscriptions);
		UpdatePoints();
	...
	private void CheckPoints()
	{
		...
		int id = _selloutMapperProvider.Get(_sellout.ID).rewards_id[^1];
	...
	private void TrackCalendarState()
	{
		...
		if (_calendarModel != null)
		{
			...
			_calendarStateStream = _calendarModel.CalendarState.Subscribe(delegate
			{
				CheckActivate();
			});
		}
