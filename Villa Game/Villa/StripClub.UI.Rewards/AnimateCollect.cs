...
public class AnimateCollect : MonoBehaviour
{
	...
	public void Launch()
	{
		if (isInited)
		{
			...
			for (int i = 0; i < ImagesCount; i++)
			{
				...
				sequence.Insert(Delay * (float)i, instance.DOColor(new Color(instance.color.r, instance.color.g, instance.color.b, 1f), 0.1f));
			}
	...
	private Vector3[] SetPath(Transform parent, Transform target)
	{
		...
		for (int i = 0; i < PointsCount; i++)
		{
			...
			Vector3 vector2 = new Vector3(Mathf.Cos(MathF.PI / 180f * num2) * num, Mathf.Cos(MathF.PI / 180f * num2) * powerY);
