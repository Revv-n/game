...
public class EventBattlePassProgressView : MonoView, IInitializable, IDisposable
{
	...
	public void Set(CalendarModel eventCalendarModel, BattlePass battlePass)
	{
		...
		_rewardUpdateStream = battlePass.OnRewardUpdate.Subscribe(delegate
		{
		...
		_premiumStream = battlePass.Data.StartData.PremiumPurchasedProperty.Subscribe(delegate(bool status)
		{
			int stateNumber = (status ? 1 : 0);
			_holderGroup.Set(stateNumber);
		});
		_calendarStateStream = _eventCalendarModel.CalendarState.Where((EntityStatus status) => status == EntityStatus.Complete).Subscribe(delegate
		{
			_holderGroup.Set(2);
		});
	}
	...
	private void ApplyData(ViewSettings? eventViewSettings, BattlePass battlePass)
	{
		...
		{
			_characterProvider.GetStory(bp_preview_girl_id).Do(delegate(CharacterStories characterStories)
			{
	...
	private void CheckLastChance()
	{
		...
		if (!_isLastChance)
		{
			...
			GreenT.HornyScapes.Events.Event @event = _eventSettingsProvider.GetEvent(event_id);
