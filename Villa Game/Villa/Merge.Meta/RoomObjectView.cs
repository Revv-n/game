...
public class RoomObjectView : MonoBehaviour, IRayClickable, IPointerClickHandler, IEventSystemHandler
{
	...
	public GreenT.HornyScapes.Animations.Animation AfterChangeAnimation => afterChangeAnimationGroup;

	public GreenT.HornyScapes.Animations.Animation BeforeChangeAnimation => beforeChangeAnimationGroup;
	...
	private List<GreenT.HornyScapes.Animations.Animation> InitAnimations(GreenT.HornyScapes.Animations.Animation prefabInstance, string name)
	{
		List<GreenT.HornyScapes.Animations.Animation> list = new List<GreenT.HornyScapes.Animations.Animation>();
		prefabInstance.name = name;
		GreenT.HornyScapes.Animations.Animation[] components = prefabInstance.GetComponents<GreenT.HornyScapes.Animations.Animation>();
		foreach (GreenT.HornyScapes.Animations.Animation animation in components)
		{
	...
	private void InitAnimation(GreenT.HornyScapes.Animations.Animation animation)
	{
		if (!(animation is TransformAnimation transformAnimation))
		{
			if (animation is AnimationGroup animationGroup)
			{
				foreach (GreenT.HornyScapes.Animations.Animation animation2 in animationGroup.Animations)
				{
